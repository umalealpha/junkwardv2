...
export default function Ledger() {
  ...
  const loadLedger = useCallback(() => {
    ...
    apiClient
      ...
      .catch(() => setError('Failed to load commission ledger.'))
              ...
              {!loading && entries.length === 0 ? (
                ...
                entries.map((entry) => (
                  <tr key={entry.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-4 py-2 font-medium text-ace-primary">{entry.policy_number}</td>
                    <td className="px-4 py-2 text-gray-700">
                      {entry.agent_id ? (
                        <Link to={`/agent/${entry.agent_id}`} className="hover:text-ace-primary hover:underline">
                          {entry.agent_name}
                        </Link>
                      ) : (
                        entry.agent_name
