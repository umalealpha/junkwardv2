...
import type { GroupPolicyFilters } from '../../api/groupPolicies'
...
export default function GroupPolicyPage() {
  ...
  const filters: GroupPolicyFilters = {
    status: searchParams.get('status') !== null ? Number(searchParams.get('status')) : undefined,
    search: searchParams.get('search') || undefined,
    per_page: 25,
    page: Number(searchParams.get('page') || '1'),
  }

  const { data, isLoading, isFetching } = useGroupPolicies(filters)
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-800">Group Policies</h1>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                ...
                <tr><td colSpan={7} className="p-0"><EmptyState compact title="No group policies found" description="Try adjusting your filters or search terms." /></td></tr>
              ) : (
                data?.data.map(item => {
