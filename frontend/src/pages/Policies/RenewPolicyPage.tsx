...
import type { RenewalFilters } from '../../api/renewals'
...
export default function RenewPolicyPage() {
  ...
  const filters: RenewalFilters = {
    is_renewed: searchParams.get('is_renewed') !== null ? Number(searchParams.get('is_renewed')) : undefined,
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="flex flex-wrap gap-3 items-end">
        <div>
          ...
          <input
            type="text"
            placeholder="Policy number, customer name, phone..."
        ...
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1">Renewed</label>
          <select
            value={searchParams.get('is_renewed') ?? ''}
            onChange={e => updateFilter('is_renewed', e.target.value)}
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-gray-600 uppercase text-xs tracking-wider">
              <tr>
                ...
                <th className="px-4 py-3 text-right">Old Premium</th>
                <th className="px-4 py-3 text-right">New Premium</th>
                <th className="px-4 py-3 text-left">Expiry Date</th>
                <th className="px-4 py-3 text-center">Rerated</th>
                <th className="px-4 py-3 text-center">Renewed</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                ...
                <tr><td colSpan={9} className="p-0"><EmptyState compact title="No renewals found" description="Try adjusting your filters." /></td></tr>
              ) : (
                data?.data.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-4 py-2 text-right">{fmtCurrency(item.oldPremium)}</td>
                    <td className="px-4 py-2 text-right">{fmtCurrency(item.newPremium)}</td>
                    <td className="px-4 py-2 text-gray-500">
                      {fmtDate(item.expiryDate)}
                    </td>
                    <td className="px-4 py-2 text-center">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${item.rerated ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
                        {item.rerated ? 'Yes' : 'No'}
                      </span>
                    </td>
                    <td className="px-4 py-2 text-center">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${item.isRenewed ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                        {item.isRenewed ? 'Yes' : 'No'}
                      </span>
                    </td>
                    <td className="px-4 py-2 text-gray-500">
