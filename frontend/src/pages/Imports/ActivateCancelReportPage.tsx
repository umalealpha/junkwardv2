...
const STATUS_BADGE: Record<number, { label: string; cls: string }> = {
  1: { label: 'Activation', cls: 'bg-green-100 text-green-700' },
  2: { label: 'Cancellation', cls: 'bg-red-100 text-red-700' },
  3: { label: 'DPO Refund', cls: 'bg-blue-100 text-blue-700' },
}

export default function ActivateCancelReportPage() {
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-800">Activate & Cancel Report</h1>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                ...
                <tr><td colSpan={5} className="p-0"><EmptyState compact title="No import records found" description="Try adjusting your filters." /></td></tr>
              ) : (
                data?.data.map(item => {
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-4 py-2 text-blue-600 truncate max-w-[250px]" title={item.uploadedFile ?? ''}>{item.uploadedFile || '\u2014'}</td>
                      <td className="px-4 py-2">
                        {badge ? (
                          <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${badge.cls}`}>{badge.label}</span>
                        ) : (
                          <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600">{item.statusLabel || 'Unknown'}</span>
                        )}
                      </td>
