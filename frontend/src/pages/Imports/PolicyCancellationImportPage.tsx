...
export default function PolicyCancellationImportPage() {
  ...
  async function handleFileUpload(file: File) {
    ...
    try {
      const result = await uploadExcelImport(file, 'cancellation')
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-800">Policy Cancellation Import</h1>
      </div>

      {/* Upload zone */}
      <div
        className={`border-2 border-dashed rounded-lg p-8 text-center transition ${dragOver ? 'border-red-400 bg-red-50' : 'border-gray-300 bg-white'} ${uploading ? 'opacity-60 pointer-events-none' : ''}`}
      ...
      >
        ...
        {uploading ? (
          ...
          <>
            <p className="text-sm text-gray-500 mb-1">Drag and drop your cancellation Excel file here, or</p>
            <button
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                ...
                <tr><td colSpan={5} className="p-0"><EmptyState compact title="No cancellation imports found" description="Try adjusting your filters." /></td></tr>
              ) : (
                data?.data.map(item => (
