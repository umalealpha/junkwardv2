...
import { useDeviceInspections } from '../../hooks/usePreinspection'
...
export default function DevicePreinspectionPage() {
  ...
  const { data, isLoading, isFetching } = useDeviceInspections(filters)
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-800">Device Preinspection</h1>
      </div>
      ...
      <div className="flex flex-wrap gap-3 items-end">
        <div>
          ...
          <input
            type="text"
            placeholder="IMEI, policy number, make, model..."
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                ...
                <tr><td colSpan={9} className="p-0"><EmptyState compact title="No device inspections found" description="Try adjusting your filters." /></td></tr>
              ) : (
                data?.data.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-4 py-2">{item.deviceValue ?? '\u2014'}</td>
                    <td className="px-4 py-2">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_BADGE[item.deviceStatusLabel] ?? 'bg-gray-100 text-gray-600'}`}>
                        {item.deviceStatusLabel}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-2 text-right">
                      <Link to={`/preinspection/device/${item.id}`} className="text-blue-600 hover:underline text-sm font-medium">View / Edit</Link>
                    </td>
                  </tr>
                ))
              )}
