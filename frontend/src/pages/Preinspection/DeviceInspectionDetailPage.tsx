...
import { useDeviceInspection, useApproveDeviceInspection } from '../../hooks/usePreinspection'
import type { DeviceApprovePayload } from '../../api/preinspection'
import LoadingSpinner from '../../components/common/LoadingSpinner'
import ImageReviewCard from './ImageReviewCard'

type CardKey = 'front' | 'back' | 'left' | 'right' | 'top' | 'bottom'

interface CardConfig {
  key: CardKey
  title: string
  statusField: keyof DeviceApprovePayload
  remarkField: keyof DeviceApprovePayload
}

const CARDS: CardConfig[] = [
  { key: 'front',  title: 'Front',  statusField: 'cell_phone_front_status',  remarkField: 'cell_phone_front_image_remark' },
  { key: 'back',   title: 'Back',   statusField: 'cell_phone_back_status',   remarkField: 'cell_phone_back_image_remark' },
  { key: 'left',   title: 'Left',   statusField: 'cell_phone_left_status',   remarkField: 'cell_phone_left_image_remark' },
  { key: 'right',  title: 'Right',  statusField: 'cell_phone_right_status',  remarkField: 'cell_phone_right_image_remark' },
  { key: 'top',    title: 'Top',    statusField: 'cell_phone_top_status',    remarkField: 'cell_phone_top_remark' },
  { key: 'bottom', title: 'Bottom', statusField: 'cell_phone_bottom_status', remarkField: 'cell_phone_bottom_remark' },
]
...
const EMPTY: DecisionMap = {
  ...
  top: { status: null, remark: '' }, bottom: { status: null, remark: '' },
}
...
export default function DeviceInspectionDetailPage() {
  ...
  const { data, isLoading, isError } = useDeviceInspection(inspectionId)
  const approve = useApproveDeviceInspection(inspectionId)
  ...
  function handleSave() {
    const payload: DeviceApprovePayload = { remark }
    for (const c of CARDS) {
    ...
    approve.mutate(payload, {
      onSuccess: (res) => {
        ...
        setTimeout(() => navigate('/preinspection/device'), 1200)
      },
      onError: (err: any) => {
  ...
  if (isError || !data) {
    return (
      <div className="p-6">
        <p className="text-red-600">Failed to load device inspection.</p>
        <Link to="/preinspection/device" className="text-blue-600 hover:underline text-sm">← Back to list</Link>
      </div>
    )
  }
  ...
  return (
    <div className="p-6 space-y-4 max-w-5xl">
      <div className="flex items-center justify-between">
        <div>
          <Link to="/preinspection/device" className="text-sm text-blue-600 hover:underline">← Back to device inspections</Link>
          <h1 className="text-2xl font-bold text-gray-800 mt-1">Device Inspection · {[data.make, data.model].filter(Boolean).join(' ') || '—'}</h1>
        </div>
      ...
      <div className="bg-white rounded-lg border shadow-sm p-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
        ...
        <div><div className="text-xs text-gray-500">IMEI</div><div className="font-medium text-gray-800">{data.imei || '—'}</div></div>
        <div><div className="text-xs text-gray-500">Device value</div><div className="font-medium text-gray-800">{data.deviceValue ?? '—'}</div></div>
      </div>
      ...
      <div className="bg-white rounded-lg border shadow-sm p-4 space-y-2">
        ...
        <p className="text-xs text-gray-500">
          When all six sides are approved the policy is activated (if a successful payment exists) and the customer is notified.
      ...
      <div className="flex gap-3">
        ...
        <Link to="/preinspection/device" className="px-4 py-2 border rounded-md text-sm text-gray-600 hover:bg-gray-50">Cancel</Link>
      </div>
    </div>
  )
}
