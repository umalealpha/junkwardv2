...
import { useDeduplication } from '../../hooks/useKyc'
import LoadingSpinner from '../../components/common/LoadingSpinner'
import type { DeduplicationFilters } from '../../api/kyc'
...
export default function DeduplicationPage() {
  ...
  const filters: DeduplicationFilters = {
  ...
  const { data, isLoading, isFetching } = useDeduplication(filters)
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-800">Deduplication</h1>
      </div>
      ...
      <div className="flex flex-wrap gap-3 items-end">
        <div>
          ...
          <input
            type="text"
            placeholder="Customer name, email, omang..."
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-gray-600 uppercase text-xs tracking-wider">
              <tr>
                ...
                <th className="px-4 py-3 text-left">Doc Upload</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                ...
                <tr><td colSpan={8} className="p-0"><EmptyState compact title="No deduplication records found" description="Try adjusting your filters." /></td></tr>
              ) : (
                data?.data.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-4 py-2">
                      {item.documentUploadStatus ? (
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize ${DOC_BADGE[item.documentUploadStatus.toLowerCase()] ?? 'bg-gray-100 text-gray-600'}`}>
                          {item.documentUploadStatus}
                        </span>
                      ) : '\u2014'}
                    </td>
                    <td className="px-4 py-2">
                      {item.verificationStatus ? (
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize ${VERIFY_BADGE[item.verificationStatus.toLowerCase()] ?? 'bg-gray-100 text-gray-600'}`}>
                          {item.verificationStatus}
                        </span>
                      ) : '\u2014'}
                    </td>
