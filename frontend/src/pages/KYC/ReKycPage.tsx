...
import type { KycFilters } from '../../api/kyc'
...
export default function ReKycPage() {
  ...
  const { data, isLoading, isFetching } = useKycList(filters)
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-800">Re-KYC Management</h1>
      </div>

      <div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 text-sm text-blue-700">
        Re-KYC campaigns are managed here. Customers due for periodic KYC re-verification are listed below.
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                ...
                <tr><td colSpan={7} className="p-0"><EmptyState compact title="No Re-KYC records found" description="Try adjusting your filters." /></td></tr>
              ) : (
                data?.data.map(item => {
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-4 py-2 truncate max-w-[180px]">{item.email || '\u2014'}</td>
                      ...
                      <td className="px-4 py-2 truncate max-w-[160px]" title={item.remark ?? ''}>{item.remark || '\u2014'}</td>
