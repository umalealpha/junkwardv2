...
export default function BatchCancelPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await apiClient.post('/batch-cancel', formData, { headers: { 'Content-Type': 'multipart/form-data' } })
      setResult({ success: true, message: 'Cancellation file uploaded successfully. Processing will begin shortly.' })
      setFile(null)
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-800">Batch Policy Cancel</h1>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border p-6 max-w-2xl">
        <div className="mb-6">
          ...
          <ul className="text-sm text-gray-600 space-y-1 list-disc list-inside">
            <li>Download the cancellation template using the button below.</li>
            <li>Fill in the policy numbers and cancellation reasons.</li>
            <li>Upload the completed file (.xlsx, .xls, or .csv).</li>
            <li>The system will validate and process cancellations in the background.</li>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            ...
            className="w-full px-4 py-2.5 bg-red-600 text-white rounded-md font-medium hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed text-sm"
          >
            {uploading ? 'Uploading...' : 'Upload & Process Cancellations'}
          </button>
        </form>
      </div>
    </div>
