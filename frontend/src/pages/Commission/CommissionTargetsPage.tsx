...
export default function CommissionTargetsPage() {
  ...
  function loadTargets() {
    ...
    apiClient
      ...
      .catch((err: any) => {
        const message = err?.response?.data?.message ?? err?.message ?? 'Failed to load commission targets.'
        setError(message)
        reportErrorToTeam({ error: message, stack: err?.stack, context: 'CommissionTargetsPage:loadTargets' })
      })
    ...
    <div className="p-6 space-y-4">
      ...
      {error && (
        <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-2 rounded-md text-sm flex items-center gap-3">
          ...
          <button onClick={loadTargets} className="px-2.5 py-1 bg-red-600 text-white rounded text-xs font-medium hover:bg-red-700">Try again</button>
      ...
      <Modal
        ...
        title={editingId ? 'Edit Commission Target' : 'Add Commission Target'}
        footer={
          <>
            <button
