...
<div id="content" style="margin-bottom: 0px;">
    @if(count($policies) > 0)
        <p style="font-size: large"> {{ count($policies) }} Policies has been cancelled today.</p>
        <table class="table-responsive" style="border-collapse: collapse;width:100%">
            ...
            @foreach($policies as $p)
                <tr>
                    ...
                    <td>
                        <p style="color: #000000; margin: 0px;">
                            <b>{{ date('Y-m-d', strtotime($p->policy_created)) }}</b>
                        </p>
                    </td>
                    <td>
                        <p style="color: #000000; margin: 0px;">
                    ...
                    <td>
                        <p style="color: #000000; margin: 0px;">
                        @php $feedback = \AlphaDirect\CustomerFeedback::where('policy_id',$p->id)->orderBy('id','desc')->first(); @endphp
                        @if($feedback && $feedback->cancelled_by != null)
                                <b>{{ $feedback->cancelled_by }}</b>
                        @else
                           <b>Customer</b>
                        @endif
                        </p>
                    ...
                    <td>
                        <p style="color: #000000; margin: 0px;">
                            @if($p->cancelled_date )
                                <b>{{ $p->cancelled_date }}</b>
                            @else
                            <b>{{ $p->policy_updated }}</b>
                        @endif
                        </p>
                    </td>
                    <td>
                        <p style="color: #000000; margin: 0px;">
                            <b>{{ $p->reason.'-'.$p->circumstances.'-'.$p->other_company }}</b>
                        </p>
                    </td>
                </tr>
            @endforeach
