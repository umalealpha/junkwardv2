...
<div id="content" style="margin-bottom: 0px;">
    ...
    @if(count($policies) > 0)
        <p style="font-size: large">Total number Deactivated policies Payments Success Policy upto Today: {{ count($policies) }} </p>
            ...
            <thead>
                <tr>
                    ...
                    <th style="background-color: #dbe5f1;">
                     ...
                     <th style="background-color: #dbe5f1;">
                        <p style="text-align: center;margin: 0px;">
                            <b>Policy Created At</b>
                        </p>
                    </th>
                    <th style="background-color: #dbe5f1;">
                        <p style="text-align: center;margin: 0px;">
                            <b>New Payment Date</b>
                        </p>
                    </th>
            ...
            @foreach($policies as $key=> $q)
                <tbody>
                    <tr>
                        ...
                        <td>
                            <p style="color: #000000; margin: 0px;">
                                <b>{{   \Carbon\Carbon::parse($q->created_at)->format('Y-m-d') }}</b>
                            </p>
                        </td>
                        <td>
                            <p style="color: #000000; margin: 0px;">
                            @php
                            $paytrx_graphite = \AlphaDirect\PaymentTransaction::where('policyNumber',$q->policyNumber)
                                ->whereIn('status',['Success','SUCCESS','success',1])->orderBy('id','asc')->get(['created_at']);
                            $paytrx_archive = \AlphaDirect\Models\PaymentTransactionArchive::where('policyNumber',$q->policyNumber)
                                ->whereIn('status',['Success','SUCCESS','success',1])->orderBy('id','asc')->get(['created_at']);
                            $merged = $paytrx_archive->merge($paytrx_graphite);
                                $trxn = $merged->all();
                            @endphp
                            @if(count($trxn) > 0)
                                <b>{{   \Carbon\Carbon::parse($trxn[0]->created_at)->format('Y-m-d') }}</b>
                            @endif
                            </p>
                        </td>
