...
class Customer extends Authenticatable implements Auditable
{
 ...
 public function transformAudit(array $data): array
    ...
    {
        return $this->hasOne('AlphaDirect\CustomerProfile', 'customer_id', 'id')->select();
    }

    public function policy()
    {
        return $this->hasMany('AlphaDirect\Policy', 'customer_id')->select();
    }

    protected function claims()
    {
        return $this->hasMany('AlphaDirect\Claim', 'customer_id')->select();
    }
    ...
    {
        return $this->hasOne('AlphaDirect\Lead', 'customerId', 'id')->select();
    }

    public function qoute()
    {
        return $this->hasOne('AlphaDirect\Quote', 'customerId', 'id')->select();
    }
        ...
        switch ($category) {
            ...
            case "cellPhone":
                ...
                return $query->with(['profile', 'policy'])
                    ->where('cellphone', 'like', '%' . $customerInfo . '%')
                     ->get();
