...
class ExpiredMotorCompPolicies extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'expiredMotorCompPolicies:cron';
     ...
     * @var string
     */
    protected $description = 'Motor comp expired policies';
    ...
    {
        $cron = new CronStatus();
        $cron->name = "expiredMotorCompPolicies:cron";
        $cron->start = \Carbon\Carbon::now();
        $cron->save();
        Log::info('Cron Started for motor comp expired policies');

        try{
            $expiredPolicies = DB::select(DB::raw('select * from policies where status = 1 and product_id = 3 and policyNumber not in (select policyNumber from expired_policies_import where is_renewed = 0);'));

            // Policy::where('product_id',3)->where('status',1)->orderby('id','desc')->chunkById(100, function($expiredPolicies)
            // {
                Log::info("chunk - ". count($expiredPolicies));
                dump("chunk - ". count($expiredPolicies));
                if (count($expiredPolicies) > 0) {
                    ...
                    foreach($expiredPolicies as $key=>$item){
                        ...
                        // if ($item->policyNumber == 'MIS2022004042') { //MIS2022004458 //MIS2021002346
                        ...
                        if ($expiryDate <= $todayDate) {
                            ...
                            if($data != null){
                                ...
                                if (isset($policy)) {
                                    $expired_policies_import = ExpiredPoliciesImportJobs::where('policyNumber',$item->policyNumber)->first();
                            ...
                            //                     // }
                                                ...
                                                $term_end_date = PolicyTerm::where('policy_id',$data->policy_id)->orderBy('id','desc')->first('term_end_date');
                                                ...
                                                if ($expired_policies_import->new_premium <= $expired_policies_import->old_premium) {
                                                    if ($expired_policies_import->paymentMethod == 'DPO' || $expired_policies_import->paymentMethod == 'VCS' || $expired_policies_import->paymentMethod == 'RealPay' || $expired_policies_import->paymentMethod == 'Realpay') {
                                                        ...
                                                        if (isset($payTrx)) {
                                                            // $autoRenewedPolicy = 1;
                                                            $expired_policies_import->can_autoRenewed = 1;
                                                        }
                                                    }
                                                }
                                                ...
                                                // if ($expired_policies_import->new_premium <= $expired_policies_import->old_premium) {
                                                //     if ($expired_policies_import->paymentMethod == 'DPO' || $expired_policies_import->paymentMethod == 'RealPay' || $expired_policies_import->paymentMethod == 'Realpay') {
                                                //         $renewPolicy = $policyController->renewMotorCompExpiredPolicy($data->policyNumber);
            ...
            $policies_expired_list = ExpiredPoliciesImportJobs::whereDate('expiry_date','<=',$todayDate)->whereDate('created_at',$todayDate)->get();
            $total_policies_expired_list = ExpiredPoliciesImportJobs::whereDate('expiry_date','<=',$todayDate)->get();

            $report = [
                'policies' => $policies_expired_list,
                'total_policies_expired_list' => $total_policies_expired_list,
                'title'    => 'Expired policies Listing'
            ];
            // return view('admin.notes.expiredPoliciesList',$report);
            $date = \Carbon\Carbon::now()->timestamp;
            $path = 'policies-'.$date.'/expiredButNeedToBeRenewed.pdf';
            libxml_use_internal_errors(true);
            $pdf = PDF::loadView('admin.notes.expiredButNeedToBeRenewed', $report)->setPaper('a3', 'landscape');
            ...
            if(count($email) > 0) {
                foreach($email as $d){
                    if($d){
                        ...
                        $sdata->hook = 'expired_but_need_to_be_renewed';
        ...
        Log::info('Cron finish for motor comp expired policies');
