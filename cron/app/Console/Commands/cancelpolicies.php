...
class cancelpolicies extends Command
{
    ...
    public function handle()
    {
        ...
        $policies = DB::select(DB::raw(
            'SELECT c.firstName,c.cellphone,p.policyNumber,p.status,p.id as policy_id,p.policyNumber as policy_number,cp.reason,cp.status as cancellation_status FROM cancel_policies cp
                    ...
                    if($data){
                        sleep(1);
                            $trans = PaymentTransaction::where('policyNumber',$data->policyNumber)
                                ...
                                if( isset($trans->paymentMethod) && !empty($trans->paymentMethod )){
                                    $check = DB::select(DB::raw("select * from customer_banking where policy_id = {$data->policy_id} order by id desc limit 0,1"));
                        ...
                        if($cancelled){
                            ...
                            if($data->cellphone){
                                ...
                                //SendSMSForCancellation($smsTemplateID,$policyNumber,$c_name,$cellphone)
                                $sms->SendSMSForCancellation( 39,$policy->policyNumber,$data->firstName,$data->cellphone);
