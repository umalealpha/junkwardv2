...
class FetchDomgComgPoliciesInstlStatus extends Command
{
    ...
    public function handle()
    {
        ...
        Log::info('Cron started for update realpay instalments in database for dom com policies');

        // $records = RealpayTransactionsExcelData::where('status','!=',3)->where('domgcomg','COMG2024105513')->get();

        $records = RealpayTransactionsExcelData::where('status', '!=', 3)->limit(300)->get();
        ...
        if (isset($records)) {
            foreach ($records as $key => $record) {
                ...
                if (isset($policy)) {
                    ...
                    $policy_action = PolicyAction::where('policy_id',$policy->id)->where('transaction_type','NEWBUSINESS')->first();

                                foreach ($getcontract['InstalmentGetResponse'] as $key => $con_instalment) {
                                    $instalment = (array) $con_instalment;

                                    $instStartDate = \Carbon\Carbon::parse($instalment['InstalmentActionDate']);
                                    ...
                                    if ($instalment['InstalmentStatus'] == "S" || $instalment['InstalmentStatus'] == "F" || $instalment['InstalmentStatus'] == "I") {
                                        ...
                                        $realpayConInstallment = RealpayContractInstallments::where('clientNumber',$instalment['ClientNumber'])->where('contractNumber',$instalment['ContractNumber'])->where('InstalmentReferenceNumber',$instalment['InstalmentReferenceNumber'])->where('InstalmentSequence',$instalment['InstalmentSequence'])->first();
                                        ...
                                        if (isset($realpayConInstallment)) {
                                            ...
                                            $new->clientNumber = $instalment['ClientNumber'];
                                            $new->contractNumber = $instalment['ContractNumber'];
                                            $new->InstalmentReferenceNumber = $instalment['InstalmentReferenceNumber'];
                                            $new->InstalmentSequence = $instalment['InstalmentSequence'];
                                            $new->CTCAmount = isset($instalment['CTCAmount']) ? $instalment['CTCAmount'] : null;
                                            $new->TrackingCode = isset($instalment['TrackingCode']) ? $instalment['TrackingCode'] : null;
                                            $new->InstalmentActionDate = $instalment['InstalmentActionDate'];
                                            $new->InstalmentAmount = $instalment['InstalmentAmount'];
                                            $new->InstalmentStatus = $instalment['InstalmentStatus'];
                                            $new->save();
                                        }
                                    }
                                }
