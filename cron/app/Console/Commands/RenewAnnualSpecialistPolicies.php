...
/**
 * Anniversary renewal for SPECIALIST products only (cron-server copy).
 *
 * Specialist twin of RenewAnnualPolicies (policy:renew-annual, products 7/8).
 * Kept separate so the two crons never overlap and either can be scheduled
 * independently via the cron_kernels table / Cron Portal:
 *
 *   - policy:renew-annual            → products 7, 8            (DOM/COM motor)
 *   - policy:renew-annual-specialist → products 16,17,18,19,20,22 (specialist)
 *
 * Specialist products carry coverages in dedicated one-to-one tables
 * (car/par/ear/travel/marine/…) that plain newPolicyAction() does NOT clone,
 * so the action is cloned with newPolicyActionReplace() — the only path that
 * also walks SpecialistCoverageRegistry via replicateSpecialistCoveragesIfMissing().
 */
class RenewAnnualSpecialistPolicies extends Command
{
     ...
     * @var string
     */
    protected $signature = 'policy:renew-annual-specialist';
     ...
     *
     * SPECIALIST ANNIVERSARY RENEW CRON — ALL CASES HANDLED
     * ─────────────────────────────────────────────────────────────────────
     ...
     *  [R9] Clones via newPolicyActionReplace() so specialist coverage tables
     *       (car/par/ear/travel/marine/…) come across — NOT newPolicyAction()
     * ─────────────────────────────────────────────────────────────────────
     *
     * @return int
     */
    ...
    {
        \Log::withContext(['cron' => 'policy:renew-annual-specialist']);
        ...
        register_shutdown_function(function () {
            ...
            if ($err !== null && \in_array($err['type'], [E_ERROR, E_PARSE, E_COMPILE_ERROR, E_CORE_ERROR, E_USER_ERROR], true)) {
                \Log::error("RenewAnnualSpecialistPolicies FATAL: {$err['message']} @ {$err['file']}:{$err['line']}");
            }
        });
    ...
    private function doHandle(CronStatus $cron)
    {
        ...
        // SPECIALIST products only (16,17,18,19,20,22)
        // ──────────────────────────────────────────────────────────────────
        ...
        Log::info("Specialist Anniversary Renew Cron started. Eligible policies: " . count($policyData));
        ...
        foreach ($policyData as $policy) {
            ...
            // cloned schedule with the SPECIALIST Rate-button recipe — the plain
            // calculatePremiumRenew sums only the standard coverage tables and
            // ignores the ten specialist one-to-one tables, so it zeroed the
            // anniversary premium (and its invoice).
            //
            ...
            // was actually operative the day before it starts. $baseAction stays
            // the DATE anchor only: it skips ENDORSE entirely, so an anniversary
            // created after a mid-term endorsement cloned pre-endorsement cover
            // and had to be repaired by hand with Refresh Endorsement.
            ...
            activity('Specialist Anniversary Renewal')
                ->performedOn($policies)
                ->log('Specialist anniversary renewal - '. $policy->policyNumber.' - '.$anniversaryFrom->format('Y-m-d').' to '.$anniversaryTo->format('Y-m-d'));


            Log::info("SPECIALIST ANNIVERSARY created | Policy {$policy->policy_id} | {$policyQuoteNo} | From: {$anniversaryFrom->format('Y-m-d')} | To: {$anniversaryTo->format('Y-m-d')} | Premium: {$lastPremiumAction->premium}");
        }

        Log::info("Specialist Anniversary Renew Cron completed. Created: {$createdCount}, Skipped: {$skippedCount}");
        ...
        $this->info("Specialist annual policy renewal task completed. Created: {$createdCount}, Skipped: {$skippedCount}");
        return 0;
    }
}
