    ...
    public function handle()
    {
        ...
        if (isset($policies)) {
            foreach ($policies as $key => $policy) {
                $scheduledTx = ScheduleTransaction::where('policy_number',$policy->policyNumber)
                ->groupBy('billing_date')->select(DB::raw('COUNT(id) as cid'))->get();                
                
                if (isset($scheduledTx)) {
                    foreach ($scheduledTx as $key => $scheduledTxdata) {
                        $cIdCount = $scheduledTxdata->cid."\n";
                        if($cIdCount > 1)
                        {
