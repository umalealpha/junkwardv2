...
class PolicyLedger extends Command
{
    ...
    /**
     * Products that must never be invoiced by this calendar-driven sweep — DomCom
     * (7,8) plus the Specialist family (16-20, 22-24). Both are billed action-wise
     * by their own renew crons, which raise exactly one invoice per ISSUED RENEW
     * action. See PolicyLedgerDaily::NO_AUTO_INVOICE_PRODUCTS — kept identical.
     */
...
// dd($policy->premium.'-'.$created_date.'-'.$diff_in_months.'-'.$first_invoice);
            ...
            Log::info($this->cronName() . ' finished');
            return 0;
        } catch (\Throwable $e) {
    ...
    /**
     * Policies to process this run. Overridden by the single-policy variant
     * to scope down to one policy instead of sweeping the whole table.
     * Returns plain stdClass rows (query builder, not Eloquent) chunked the
     * same way the foreach($policies as $records) { foreach($records as $policy) }
     * loop above expects.
     */
    protected function selectPolicies(Carbon $today)
    {
        return DB::table('policies')
            ->select(
                ...
                'policyNumber',
                'policyActivatedDate',
                'is_sys_act_generated',
                'billingStartDate',
                'ori_billingStartDate',
                'status'
            )
            ...
            ->where(function ($query) use ($today) {
                ...
                $query->whereDate('created_at', $today)
                    // GRA-0117 permanent fix — same DAY-OF-MONTH re-selection as
                    // PolicyLedgerDaily (kept in sync so the weekly Sunday sweep
                    // and the daily 20:40 run agree on which policies are "due"
                    // this month). A frozen, never-advancing billingStartDate now
                    // still re-matches every month WITHOUT mutating any column.
                    //
                    // Daily-vs-weekly double-post is prevented by the shared
                    // (policy_id,'Invoice',invoice_date) dup-guard in the engine:
                    // both commands anchor at (last invoice + 1 month) and post
                    // the same invoice_date, so whichever runs first posts it and
                    // the other is a no-op. See billingDayOfMonthPredicate().
     ...
     * PolicyLedgerDaily implementation so the daily and weekly crons select the
     * same set; see the longer rationale there.
     */
