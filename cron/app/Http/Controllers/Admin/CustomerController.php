...
class CustomerController extends Controller
{
    ...
    public function data(Request $request)
    {
        ...
        foreach ($records as $record) {
            ...
            if ($record['category_reason'] != null) {
                $data = \Opis\Closure\unserialize($record['category_reason']);
    ...
    public function store(Request $request)
    {
        ...
        if (Customer::where('is_blocked',1)->where('firstName', $first_name)->where('lastName', $last_name)->exists()) {
            return Redirect::route('admin.customer.index')->with('error', 'Customer in block list');
        }elseif (Customer::where('cellphone', $cellphone)->exists()) {
            return Redirect::route('admin.customer.index')->with('error', 'Customer cellphone already exist');
        }elseif($omang != '' && CustomerProfile::where('omang', $omang)->exists()){
            return Redirect::route('admin.customer.index')->with('error', 'Customer omang already exist');
        }elseif($passport != '' && CustomerProfile::where('passport', $passport)->exists()){
 ...
 */
       ...
       // dd($customerProfile->sourceOfIncome);
        ...
        $reason = "";
            if ($customer['category_reason'] != null) {
                $data = \Opis\Closure\unserialize($customer['category_reason']);
                foreach ($data as $key => $d) {
                    $reason .=  $d;
                }
            } else {
    ...
    public function verifyKYCInfo(Request $request)
    {
        ...
        if (isset($request->data_id)) {
            ...
            if ($data->isDirty('omangFrontRemark')) {
            ...
            if ($has_vehicle == 1 && $data->driving_license == null)
            ...
            if ($records != null) {
                ...
                foreach ($records as $record) {
                    ...
                    if ($record->plan_id == 3) {
                        ...
                        $resultArr['amount'] = $paymentC->getPremiumByProductPlan($record->plan_id);
                    }
        ...
        try {
            if ($record != null) {
                   ...
                   # $resultArr['creditorName'] = $record->firstName." ".$record->middleName." ".$record->lastName;
                    ...
                    $resultArr['creditorId'] = "uwPtRrxSQs" ;#$record->customer_id;
                    $resultArr['mandateId']=  $record->policyNumber;
                    $resultArr['paymentDate'] = date('Y-m-d', strtotime('+7 days'));
