@php
    ...
    // policy. The original whereNotNull('policy_coverage_id') filter hid any
    // record saved with policy_coverage_id NULL (legacy save path / direct-
    // navigation to the schedule without a ?policy_coverage_id= URL param),
    // producing the "No Marine Cargo Open data available" placeholder even
    // though the row existed in marine_cargo_open_coverages.
    // Scope to the selected transaction's coverages (resolved in the job and
    // passed as $specialistPcIds) so each declaration shows once. Fall back to
    // policy-wide only if not provided, so the schedule never goes blank.
    $marineCargoOpenPolicyId = data_get($policy ?? null, 'id');
    $marineCargoOpenPcIds = !empty($specialistPcIds)
        ? $specialistPcIds
        : ($marineCargoOpenPolicyId
            ? \DB::table('policy_coverages')->where('policy_id', $marineCargoOpenPolicyId)->whereNull('deleted_at')->pluck('id')->all()
            : []);
    $marineCargoOpenCoverages = !empty($marineCargoOpenPcIds)
        ? \AlphaDirect\Models\MarineCargoOpenCoverage::whereIn('policy_coverage_id', $marineCargoOpenPcIds)->get()
        : collect();

    if (!function_exists('mcop_decode_items')) {
        function mcop_decode_items($value) {
    ...
    if (!function_exists('mcop_fmt_amount')) {
        function mcop_fmt_amount($value) {
            if ($value === null || $value === '') return '';
...
<table style="margin-top: 8px;">
    <tr class="section-title text-center">
        <td colspan="4">Marine Cargo — Open Policy</td>
    </tr>
</table>

@if($marineCargoOpenCoverages->isEmpty())
    <table style="margin-top: 6px;">
        <tr>
            <td class="text-center">No Marine Cargo Open data available for this policy.</td>
        </tr>
    </table>
@else
    @foreach($marineCargoOpenCoverages as $mcop)
        @php
            ...
            $clausesRaw = mcop_decode_items($mcop->clauses);
            $clauses = [];
            ...
            // Items still resolve. policy_specified_items.policy_coverage_id references
            // policy_coverages.id == $coverages->id. Mirrors travel_insurance_pdf.
            $policyCoverageKey = $mcop->policy_coverage_id ?: ($coverages->id ?? null);
        ...
        <table style="margin-top: 6px;">
            <tr class="section-subtitle">
                <td colspan="4"><strong>MARINE CARGO — Open Policy Schedule</strong></td>
            </tr>
            <tr>
                <td style="width: 25%; font-weight: bold;">Name of Assured</td>
                <td style="width: 25%;">{{ $mcop->assured_name ?? '' }}</td>
                <td style="width: 25%; font-weight: bold;">Open Policy No.</td>
                <td style="width: 25%;">{{ $mcop->open_policy_no ?? '' }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">Address of Assured</td>
                <td colspan="3">{{ $mcop->assured_address ?? '' }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">Agent / Broker Code No.</td>
                <td>{{ $mcop->agent_broker_code ?? '' }}</td>
                <td style="font-weight: bold;">Conveyance</td>
                <td>{{ $mcop->conveyance ?? '' }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">Policy Period From</td>
                <td>{{ $mcop->policy_period_from ? \Carbon\Carbon::parse($mcop->policy_period_from)->format('d/m/Y') : '' }}</td>
                <td style="font-weight: bold;">Policy Period To</td>
                <td>{{ $mcop->policy_period_to ? \Carbon\Carbon::parse($mcop->policy_period_to)->format('d/m/Y') : '' }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">Voyage From</td>
                <td>{{ $mcop->voyage_from ? \Carbon\Carbon::parse($mcop->voyage_from)->format('d/m/Y') : '' }}</td>
                <td style="font-weight: bold;">Voyage To</td>
                <td>{{ $mcop->voyage_to ? \Carbon\Carbon::parse($mcop->voyage_to)->format('d/m/Y') : '' }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">Commodities Covered</td>
                <td colspan="3">{{ $mcop->commodities_covered ?? '' }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">Nature of Packing</td>
                <td>{{ $mcop->nature_of_packing ?? '' }}</td>
                <td style="font-weight: bold;">Terms of Cover</td>
                <td>{{ $mcop->terms_of_cover ?? '' }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">Annual Estimated Turnover</td>
                <td>{{ mcop_fmt_amount($mcop->annual_estimated_turnover ?? '') }}</td>
                <td style="font-weight: bold;">Location Limit</td>
                <td>{{ mcop_fmt_amount($mcop->location_limit ?? '') }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">Premium Rate (%)</td>
                <td>{{ $mcop->premium_rate ?? '' }}</td>
                <td style="font-weight: bold;">Sum Insured</td>
                <td>{{ mcop_fmt_amount($mcop->sum_insured ?? '') }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">Premium</td>
                <td>{{ mcop_fmt_amount($mcop->premium ?? '') }}</td>
                <td style="font-weight: bold;">Today's Date</td>
                <td>{{ $mcop->today_date ? \Carbon\Carbon::parse($mcop->today_date)->format('d/m/Y') : '' }}</td>
            </tr>
        </table>
        ...
        <table style="margin-top: 6px;">
            ...
            <tr>
                ...
                <td style="white-space: pre-wrap;">{{ $mcop->survey_claim_settlement ?? '' }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">Claim Payable at</td>
                <td>{{ $mcop->claim_payable_at ?? '' }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">Claim Payable by</td>
                <td>{{ $mcop->claim_payable_by ?? '' }}</td>
            </tr>
        </table>
        ...
        <table style="margin-top: 6px;">
            ...
            <tr>
                ...
                <td>{{ $mcop->signing_date ? \Carbon\Carbon::parse($mcop->signing_date)->format('d/m/Y') : '' }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">Examined By</td>
                <td>{{ $mcop->examined_by ?? '' }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">Declaration</td>
                <td style="white-space: pre-wrap;">{{ $mcop->declaration ?? '' }}</td>
            </tr>
        </table>
        ...
        <table style="margin-top: 6px;">
            ...
            <tr>
                ...
                <td style="white-space: pre-wrap;">{{ $mcop->basis_of_valuation ?? '' }}</td>
            </tr>
        </table>
        ...
        <table style="margin-top: 6px;">
            ...
            <tr>
                ...
                <td style="white-space: pre-wrap;">{{ $mcop->deductible ?? '' }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">Notice of Cancellation</td>
                <td style="white-space: pre-wrap;">{{ $mcop->notice_of_cancellation ?? '' }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">Refund</td>
                <td style="white-space: pre-wrap;">{{ $mcop->refund ?? '' }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">Over-declaration</td>
                <td style="white-space: pre-wrap;">{{ $mcop->over_declaration ?? '' }}</td>
            </tr>
        </table>

        @if(!empty(trim((string) ($mcop->notes ?? ''))))
            <table style="margin-top: 6px;">
                <tr class="section-subtitle">
                    <td><strong>Additional Notes</strong></td>
                </tr>
                <tr>
                    <td style="white-space: pre-wrap;">{{ $mcop->notes }}</td>
                </tr>
            </table>
        @endif
            ...
            <table style="margin-top: 6px;">
                ...
                @foreach($specifiedItems as $specifiedItem)
                    ...
                    <tr>
                        ...
                        <td>{{ mcop_fmt_amount($specifiedItem->sum_insured ?? '') }}</td>
                        <td>{{ $specifiedItem->rate ?? '' }}</td>
                        <td>{{ mcop_fmt_amount($specifiedItem->calculated_value ?? '') }}</td>
                    </tr>
                @endforeach
                <tr>
                    <td colspan="3" style="font-weight: bold; text-align: right;">Miscellaneous Items Subtotal</td>
                    <td style="font-weight: bold;">{{ mcop_fmt_amount($miscSubtotal) }}</td>
                </tr>
            </table>
        @endif
    @endforeach
