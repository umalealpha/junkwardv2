...
@php
    ...
    $riskAdress = \AlphaDirect\Models\RiskAddress::where('policy_id', data_get($policy ?? null, 'id'))->latest()->first();
    $getCountry = \AlphaDirect\Country::where('id', data_get($riskAdress, 'state.country_id'))->first();
    $physical_address = data_get($riskAdress, 'physical_address');
    $city = data_get($riskAdress, 'city.name');
    $state = data_get($riskAdress, 'state.name');
    $country = data_get($getCountry, 'name');
    
    $insuredAddress = trim(($physical_address ? $physical_address . ', ' : '') .
        ($city ? $city . ', ' : '') .
        ($state ? $state . ', ' : '') .
        ($country ?: ''));
...
<div class="certificate">
    ...
    <table class="row" style="width: 100%; margin-top: 14px;">
        ...
        <tr>
            ...
            <td class="value">
                @if(is_numeric($getlimitIndemnityCert))
                    P {{ number_format((float) $getlimitIndemnityCert, 2) }}
                @else
                    {{ $getlimitIndemnityCert ?: '__________________' }}
                @endif
            </td>
        </tr>
        <tr>
