...
<body>
    <div id="content" style="margin-bottom: 0px;">
        ...
        <p style="margin: 0px;" align="right" >As On: {{ $today }}</p>
            ...
            {{-- Insured vehicle(s) — MIS motor (products 2/3) store the vehicle
                 on policy->vehicle, not as a coverage entity, so the entity
                 plate block further down never renders it. When the controller
                 passes vehicle data (cover note for motor products), print a
                 dedicated Registration Number table so the plate appears. --}}
            @php
                $coverNoteVehicles = (isset($vehicles) && $vehicles && $vehicles->count() > 0)
                    ? $vehicles
                    : ((isset($vehicle) && $vehicle) ? collect([$vehicle]) : collect());
            @endphp
            @if($coverNoteVehicles->count() > 0)
            <table class="table-responsive" style="border-collapse: collapse;width:100%;margin-bottom:15px;">
                <tr>
                    <td style="background-color: #002060;text-align:center" colspan="4">
                        <p style="margin: 0px;color:#ffffff;"><b>Insured Vehicle(s)</b></p>
                    </td>
                </tr>
                <tr>
                    <td style="background-color: #dbe5f1;"><p style="text-align:center;margin:0px;"><b>YEAR</b></p></td>
                    <td style="background-color: #dbe5f1;"><p style="text-align:center;margin:0px;"><b>MAKE</b></p></td>
                    <td style="background-color: #dbe5f1;"><p style="text-align:center;margin:0px;"><b>MODEL</b></p></td>
                    <td style="background-color: #dbe5f1;"><p style="text-align:center;margin:0px;"><b>REG NUMBER</b></p></td>
                </tr>
                @foreach($coverNoteVehicles as $veh)
                <tr>
           ...
           <!-- ======== Start Details =================== -->
            <table class="table-responsive" style="border-collapse: collapse;width:100%">
             ...
             @foreach($policy_coverages as $index => $coverages)
                    @if(isset($coverages->coverage) && $coverages->coverage->s_ScreenName != $previousScreenName)
                    <tr>
                        <td style="background-color: #002060;text-align:center" colspan="4">
                            <p style="margin: 0px;color:#ffffff;"><b>  {{$coverages->coverage->s_ScreenName??""}}  Section</b></p>
                        </td>
                    </tr>
                    <tr>
                        <!-- Risk Address: -->
                    ...
                    @php $previousScreenName = $coverages->coverage->s_ScreenName ?? $previousScreenName; @endphp
                    @endif
                ...
                @if(count($coverages->all_sub_coverages) > 0 || count($coverages->coverageDetail) > 0)
                    ...
                    @foreach($coverages->all_sub_coverages as $newSubIndex => $all_sub_coverages)
                        ...
                        {{-- {{ dd($sub_coverages) }} --}}
                            ...
                            @if(isset($all_sub_coverages->id) && isset($sub_coverages->coverage) && $all_sub_coverages->id == $sub_coverages->coverage->id)
                                @php
                                    ...
                                    <td style="" colspan="2">
                                        <p style="margin: 0px;"><b>
                                            ...
                                            @else
                                                ...
                                                @if(isset($sub_coverages->coverage) && $sub_coverages->coverage->s_CoverageCode == 'BUILDING')
                ...
                {{-- specified items --}}
                    @if(count($coverages->specifedItems)>0)
                                <tr>
                                    <td style="background-color: #002060;text-align:center" colspan="4">
                                        <p style="color: #ffffff;margin: 0px;">
                                            <b>Specified Items of {{$coverages->coverage->s_ScreenName??""}} Coverage</b>
                                        </p>
                                    </td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    <td style ="margin: 0px;" colspan="2">{{ optional($specifed_items->specifiedCoveragesItems)->specified_name ?? ($specifed_items->custom_name ?? '') }}</td>
