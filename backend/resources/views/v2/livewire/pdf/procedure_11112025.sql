CREATE DEFINER=`graphitebwlive`@`%` PROCEDURE `temp_summary_age_analyst_com`(in fd VARCHAR(110))
BEGIN
	...
	OPEN po;
       getPo: LOOP
           ...
           # AND pl.deleted_at IS NULL
			...
			set xpaymentTotal=0; #and created_at  <= '2025-08-23' 
			select sum(amount) as b from payment_transactions where policyNumber=xpolicyNumber  and  ( date(deleted_at) >= '2025-08-23' or deleted_at is null) and status="Success" and CompanyRef IS NULL and amount != 1 and is_refund = 0 and date(paymentDate) <= fd into xpaymentTotal;
			set xrefundTotal=0;
