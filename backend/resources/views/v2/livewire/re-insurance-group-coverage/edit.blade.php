<div>
    <x-slot name="breadcrum">
        ...
        <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
            <!--begin::Title-->
            <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Edit Reinsurance Group Coverage</h1>
            <!--end::Title-->
            <!--begin::Breadcrumb-->
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    @if(count($this->seletedCoverages))
                        <div style="background: #ffeb3b; padding: 10px; margin: 10px 0; border-radius: 4px;">
                            <strong>DEBUG: {{ count($this->seletedCoverages) }} parent coverages loaded</strong>
                            <br>Coverages: {{ implode(', ', array_values($this->seletedCoverages)) }}
                        </div>
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="table-responsive">
                                    <table class="table align-middle table-row-dashed fs-6 gy-5 dataTable no-footer">
                                        ...
                                        <!-- Total selected coverages: {{ count($this->seletedCoverages) }} -->
                                        @foreach($this->seletedCoverages as $selectedId => $coverageCode)
                                        <!-- Processing coverage: {{ $coverageCode }} -->
                                            <tr class="bg-light" width="100%" style="background-color: #f5f5f5;">
                                                <td colspan=5 style="color:#000;padding: 12px 8px;font-weight: 600;font-size: 15px;border-top: 2px solid #ddd;border-bottom: 1px solid #ddd;">
                                                    {{ $coverageCode ?? ""}}
                                                </td>
                                            </tr>
                                            @php
                                                $allSubcoverages = $this->getProductSubCoverages($coverageCode);
                                            @endphp
                                            @if(count($allSubcoverages) > 0)
                                                @foreach($allSubcoverages as $subcoverage)
                                                @php
                                                    $sdata = $this->getSubCoverageData($this->reinsurancegroup->id,$subcoverage->id);
                                                @endphp
                                                <tr width="100%" >
                                                    ...
                                                    <td width="20%" style="padding-top: 0;padding-bottom: 0;">
                                                        <div class="form-floating mb-3">
                                                            <select class = 'form-select form-select-solid' name="{{ $subcoverage->id }}_FIELD1" aria-label="Select SI/PREMIUM">
                                                                ...
                                                                <option value="1" {{($sdata && $sdata->si_premium=="1")?'selected':''}}>SumInsured</option>
                                                                <option value="2" {{($sdata && $sdata->si_premium=="2")?'selected':''}} >Premium</option>
                                                            </select>
                                                    ...
                                                    <td width="20%" style="padding-top: 0;padding-bottom: 0;">
                                                        <div class="form-floating mb-3">
                                                            <select class = 'form-select form-select-solid show_hide_dropdown'  onChange="show_hide_dropdown(this,{{$subcoverage->id}})"  name="{{ $subcoverage->id }}_FIELD2" aria-label="Select RI LIMIT">
                                                                <option value="">- Select -</option>
                                                                <option value="1" {{($sdata && $sdata->ri_limit=="1")?'selected':''}}>SumInsured</option>
                                                                <option value="2" {{($sdata && $sdata->ri_limit=="2")?'selected':''}}>Skip</option>
                                                                <option value="3" {{($sdata && $sdata->ri_limit=="3")?'selected':''}}>Other</option>
                                                            </select>
                                                            <x-form-label for="{{ $subcoverage->id }}_FIELD2" value="{{ __('Select RI LIMIT') }}"/>
                                                            <x-form-input-error name="{{ $subcoverage->id }}_FIELD2"/>
                                                        </div>
                                                    </td>
                                                    <td width="20%" style="padding-top: 0;padding-bottom: 0;{{($sdata && $sdata->ri_limit=='3')?'':'display:none;'}}" id="show_hide_dropdown_{{ $subcoverage->id }}">
                                                        <div class="form-floating mb-3">
                                                            <input type="text" class="form-control" name="{{$subcoverage->id}}_FIELD3"
                                                            id="{{$subcoverage->id}}_FIELD4"  value="{{($sdata && $sdata->limit_value)?$sdata->limit_value:''}}" x-mask:dynamic="$money($input)"/>
                                            ...
                                            @php
                                            $i = 0;
                                            $sdata = $this->getSubCoverageDataMotor($this->reinsurancegroup->id,22);
                                            ...
                                            @php
                                            $commsi_premium = isset($sdata[$key]['si_premium']) ? $sdata[$key]['si_premium'] : '';
                                            $commri_limit = isset($sdata[$key]['ri_limit']) ? $sdata[$key]['ri_limit'] : '';
                                            @endphp
                                            <tr width="100%">
                                                        <td width="20%" style="padding-top: 0;padding-bottom: 0;"> 
                                                        ...
                                                        <td width="20%" style="padding-top: 0;padding-bottom: 0;">
                                                            <div class="form-floating mb-3">
                                                                <select class = 'form-select form-select-solid' name="{{ $key }}_si_premium_motor_comm" aria-label="Select SI/PREMIUM">
                                                                    <option value="">- Select -</option>
                                                                    <option value="1" {{($commsi_premium == "1")?'selected':''}}>SumInsured</option>
                                                                    <option value="2" {{($commsi_premium == "2")?'selected':''}}>Premium</option>
                                                                </select>
                                                        ...
                                                        <td width="20%" style="padding-top: 0;padding-bottom: 0;">
                                                            <div class="form-floating mb-3">
                                                                <select class = 'form-select form-select-solid show_hide_dropdown' onChange="show_hide_dropdown(this,{{$key}})" name="{{ $key }}_ri_limit_motor_comm" aria-label="Select RI LIMIT">
                                                                    <option value="">- Select -</option>
                                                                    <option value="1" {{($commri_limit == "1")?'selected':''}}>SumInsured</option>
                                                                    <option value="2" {{($commri_limit == "2")?'selected':''}}>Skip</option>
                                                                    <option value="3" {{($commri_limit == "3")?'selected':''}}>Other</option>
                                                                </select>
                                            ...
                                            @php
                                            $i = 0;
                                            $sdata = $this->getSubCoverageDataMotor($this->reinsurancegroup->id,15);
                                            ...
                                            @endphp
                                                ...
                                                @php
                                                $extMotorsi_premium = isset($sdata[$key]['si_premium']) ? $sdata[$key]['si_premium'] : '';
                                                $extMotorri_limit = isset($sdata[$key]['ri_limit']) ? $sdata[$key]['ri_limit'] : '';
                                                @endphp           
                                                <tr width="100%">
                                                        <td width="20%" style="padding-top: 0;padding-bottom: 0;"> 
                                                        ...
                                                        <td width="20%" style="padding-top: 0;padding-bottom: 0;">
                                                            <div class="form-floating mb-3">
                                                                <select class = 'form-select form-select-solid' name="{{ $key }}_si_premium_motro_traders_ext" aria-label="Select SI/PREMIUM">
                                                                    <option value="">- Select -</option>
                                                                    <option value="1" {{($extMotorsi_premium == "1")?'selected':''}}>SumInsured</option>
                                                                    <option value="2" {{($extMotorsi_premium == "2")?'selected':''}}>Premium</option>
                                                                </select>
                                                        ...
                                                        <td width="20%" style="padding-top: 0;padding-bottom: 0;">
                                                            <div class="form-floating mb-3">
                                                                <select class = 'form-select form-select-solid show_hide_dropdown' onChange="show_hide_dropdown(this,{{$key}})" name="{{ $key }}_ri_limit_motro_traders_ext" aria-label="Select RI LIMIT">
                                                                    <option value="">- Select -</option>
                                                                    <option value="1" {{($extMotorri_limit == "1")?'selected':''}}>SumInsured</option>
                                                                    <option value="2" {{($extMotorri_limit == "2")?'selected':''}}>Skip</option>
                                                                    <option value="3" {{($extMotorri_limit == "3")?'selected':''}}>Other</option>
                                                                </select>
                                            ...
                                            @php
                                            $i = 0;
                                            $sdata = $this->getSubCoverageDataMotor($this->reinsurancegroup->id,16);
                                            ...
                                            @endphp
                                                ...
                                                @php
                                                $intMotorsi_premium = isset($sdata[$key]['si_premium']) ? $sdata[$key]['si_premium'] : '';
                                                $intMotorri_limit = isset($sdata[$key]['ri_limit']) ? $sdata[$key]['ri_limit'] : '';
                                                @endphp      
                                                <tr width="100%">
                                                <td width="20%" style="padding-top: 0;padding-bottom: 0;"> 
                                                ...
                                                <td width="20%" style="padding-top: 0;padding-bottom: 0;">
                                                    <div class="form-floating mb-3">
                                                        <select class = 'form-select form-select-solid' name="{{ $key }}_si_premium_motor_traders_int" aria-label="Select SI/PREMIUM">
                                                            <option value="">- Select -</option>
                                                            <option value="1" {{($intMotorsi_premium == "1")?'selected':''}}>SumInsured</option>
                                                            <option value="2" {{($intMotorsi_premium == "2")?'selected':''}}>Premium</option>
                                                        </select>
                                                ...
                                                <td width="20%" style="padding-top: 0;padding-bottom: 0;">
                                                    <div class="form-floating mb-3">
                                                        <select class = 'form-select form-select-solid show_hide_dropdown' onChange="show_hide_dropdown(this,{{$key}})" name="{{ $key }}_ri_limit_motor_traders_int" aria-label="Select RI LIMIT">
                                                            <option value="">- Select -</option>
                                                            <option value="1" {{($intMotorri_limit == "1")?'selected':''}}>SumInsured</option>
                                                            <option value="2" {{($intMotorri_limit == "2")?'selected':''}}>Skip</option>
                                                            <option value="3" {{($intMotorri_limit == "3")?'selected':''}}>Other</option>
                                                        </select>
                                            ...
                                            @endphp
                                                @endforeach
                                            @elseif($coverageCode == 'FIDELITYGUARANTEE')
                                            @php
                                            $sdata = $this->getSubCoverageDataMotor($this->reinsurancegroup->id,15);
                                            
                                                $fidelityguarantee = array(
                                                    "blanket" => "Blanket", 
                                                    "name_and_position" => 'Name And Position'
                                                    );                                                                                         
                                            @endphp
                                                @foreach($fidelityguarantee as $key => $subcoverage) 
                                                
                                                @php
                                                $intFidelitysi_premium = isset($sdata[$key]['si_premium']) ? $sdata[$key]['si_premium'] : '';
                                                $intFidelityri_limit = isset($sdata[$key]['ri_limit']) ? $sdata[$key]['ri_limit'] : '';
                                                @endphp 
                                                <tr width="100%">
                                                <td width="20%" style="padding-top: 0;padding-bottom: 0;"> 
                                                <input type="hidden" name="{{ $key }}_coverage_name_fidelityguarantee" value="{{ $subcoverage }}"/>    
                                                {{ $subcoverage }}</td>
                                                <td width="20%" style="padding-top: 0;padding-bottom: 0;">
                                                    <div class="form-floating mb-3">
                                                        <select class = 'form-select form-select-solid' name="{{ $key }}_si_premium_fidelityguarantee" aria-label="Select SI/PREMIUM">
                                                            <option value="">- Select -</option>
                                                            <option value="1" {{($intFidelitysi_premium == "1")?'selected':''}}>SumInsured</option>
                                                            <option value="2" {{($intFidelitysi_premium == "2")?'selected':''}}>Premium</option>
                                                        </select>
                                                        <x-form-label for="{{ $key }}_si_premium_fidelityguarantee" value="{{ __('Select SI/PREMIUM') }}"/>
                                                        <x-form-input-error name="{{ $key }}_si_premium_fidelityguarantee"/>
                                                    </div>
                                                </td>
                                                <td width="20%" style="padding-top: 0;padding-bottom: 0;">
                                                    <div class="form-floating mb-3">
                                                        <select class = 'form-select form-select-solid show_hide_dropdown' onChange="show_hide_dropdown(this,{{$key}})" name="{{ $key }}_ri_limit_fidelityguarantee" aria-label="Select RI LIMIT">
                                                            <option value="">- Select -</option>
                                                            <option value="1" {{($intFidelityri_limit == "1")?'selected':''}}>SumInsured</option>
                                                            <option value="2" {{($intFidelityri_limit == "2")?'selected':''}}>Skip</option>
                                                            <option value="3" {{($intFidelityri_limit == "3")?'selected':''}}>Other</option>
                                                        </select>
                                                        <x-form-label for="{{ $key }}_ri_limit_fidelityguarantee" value="{{ __('Select RI LIMIT') }}"/>
                                                        <x-form-input-error name="{{ $key }}_ri_limit_fidelityguarantee"/>
                                                    </div>
                                                </td>
                                                <td width="20%" style="padding-top: 0;padding-bottom: 0;display:none;" id="show_hide_dropdown_{{ $key }}">
                                                    <div class="form-floating mb-3">
                                                        <input type="text" class="form-control" name="{{$key}}_limit_value_fidelityguarantee"
                                                        id="{{$key}}_limit_value_fidelityguarantee"  value="" x-mask:dynamic="$money($input)"/>
                                                        <x-form-label for="{{$key}}_limit_value_fidelityguarantee" required value="{{ __('Limit') }}"/>
                                                        <x-form-input-error name="{{$key}}_limit_value_fidelityguarantee"/>
                                                    </div>
                                                </td>
                                                <td width="20%" style="padding-top: 0;padding-bottom: 0;">
                                                </td>
