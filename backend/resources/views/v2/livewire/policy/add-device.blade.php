                    ...
                    <div class="col-sm-12">
                        <div class="table-responsive">
                            <table class="table align-middle table-row-dashed fs-6 gy-5 dataTable no-footer">
                                ...
                                @php $i=1;@endphp
                                @forelse($this->allDevices as $device)
                                    <tr class="{!! ($loop->iteration % 2 == 0)?'even':'odd' !!}">
                                        ...
                                        <td>{{ number_format($device->phone_value ?? "", 2, '.', ',')}}</td>
                                        <td>{{$device->risk->address_name ?? ""}}</td>
                                        @if($editable)
                                        <td>
                                            <div class="flex space-x-1 justify-around">
													<span class="svg-icon svg-icon-2 m-0"><button wire:click.prevent="deviceDetailsedit('{{\Crypt::encrypt($device->id)}}')"  class="btn  btn-sm btn-outline btn-outline-dashed hover-scale btn-outline-primary btn-active-light-primary">
													...
													<button onclick="deleteRow('triggerDeviceDelete','{{$device->id}}')"  class="btn  btn-sm btn-outline btn-outline-dashed hover-scale btn-outline-danger btn-active-light-danger">
        ...
        <div class="col-sm-12">
            <div class='card card-custom gutter-b'>
                ...
                <div class="card-body" x-data="{
                    ...
                    <livewire:common.excel-import  :policy="$policy" :selectedType="'device'" :termId="$termId" :actionId="$actionId" :exportLinkName="'Device Data'"/>
                    <br>
                    <div class="row">
                        <div class="col-sm-3">
                    ...
                    <div class="row">
                        <div class="col-sm-3">
                            <div class="form-floating mb-3">
                                <x-form-input type="text" wire:model.defer="deviceData.phone_value" placeholder="Please enter the amount." x-mask:dynamic="$money($input)"/>
                        ...
                        <div class="col-sm-3">
                            <div class="form-floating mb-3">
                                <x-select-search wire:model.defer="deviceData.risk_id"  id="risk_id"
                                    aria-label="Select Risk Address" :options="$this->getallRiskAddress()??[]"	/>
                                <x-form-label for="deviceData.risk_id" required value="{{ __('Select Risk Address') }}" />
                                <x-form-input-error name="deviceData.risk_id"/>
                            </div>
                        </div>
