<div wire:loading.class="page-loading">
                    ...
                    <div class="col-sm-12">
                        <div class="table-responsive">
                            <table class="table align-middle table-row-dashed fs-6 gy-5 dataTable no-footer">
                                ...
                                @php $i=1;@endphp
                                @forelse($this->AllBeneficiaries as $bri)
                                    <tr class="{!! ($loop->iteration % 2 == 0)?'even':'odd' !!}">
                                        ...
                                        <td>{{ number_format($bri->payment ?? "", 2, '.', ',')}}</td>
                                        <td style="text-transform: uppercase">{{$bri->passport ?? ""}}</td>
                                        <td style="text-transform: uppercase">{{$bri->omang ?? ""}}</td>
                                        <td>
                                            @if(isset($bri->dob))
                                                {{ \Carbon\Carbon::parse($bri->dob)->format('d/m/Y')}}
                                            @endif
                                        </td>
                                        <td>{{$bri->risk->address_name ?? ""}}</td>
                                        @if($editable)
                                            <td>
                                                <div class="flex space-x-1 justify-around">
                                                    <button wire:click.prevent="beneficiaryDetailsedit('{{\Crypt::encrypt($bri->id)}}')"  class="btn  btn-sm btn-outline btn-outline-dashed hover-scale btn-outline-primary btn-active-light-primary">
                                                    ...
                                                    <button onclick="deleteRow('beneficiaryTriggerDelete','{{$bri->id}}')"  class="btn  btn-sm btn-outline btn-outline-dashed hover-scale btn-outline-danger btn-active-light-danger">
    ...
    @if($this->termId and $this->editable)
    <hr>
    <div class="row">
        <div class="col-sm-12">
            <div class='card card-custom gutter-b'>
                ...
                <div class="card-body">
                    <livewire:common.excel-import  :policy="$policy" :selectedType="'member'" :termId="$termId" :actionId="$actionId" :exportLinkName="'Beneficiary Data'" />
                    <br>
                    <div class="row">
                        <div class="col-sm-3">
                        ...
                        <div class="col-sm-3">
                            <div class="form-floating mb-3">
                                ...
                                <x-form-label for="relation" required value="{{ __('Beneficiary Relation') }}"/>
                        ...
                        <div class="col-sm-3">
                            <div class="form-floating mb-3">
                                ...
                                <x-form-label for="gender"  value="{{ __('Select Gender') }}"/>
                        ...
                        <div class="col-sm-3">
                            <div class="form-floating mb-3">
                                <x-form-date type="text" class="kt_datepicker_1" id="beneficiaryData_dob" wire:model="beneficiary_date_of_birth" placeholder="Date Of Birth"/>
                                <x-form-label for="beneficiaryData_dob" required value="{{ __('Date Of Birth') }}"/>
                                <x-form-input-error name="beneficiary_date_of_birth"/>
                            </div>
                        </div>
                        <div class="col-sm-3">
                            <div class="form-floating mb-3">
                                <x-select-search wire:model.defer="beneficiaryData.risk_id"  id="risk_id"
                                    aria-label="Select Risk Address" :options="$this->allRiskaddress??[]"	/>
                                <x-form-label for="beneficiaryData.risk_id" required value="{{ __('Select Risk Address') }}" />
                                <x-form-input-error name="beneficiaryData.risk_id"/>
                            </div>
                        </div>
                        <!-- <div wire:loading >
