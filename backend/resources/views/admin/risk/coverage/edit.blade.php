...
class="kt-header--fixed kt-header-mobile--fixed kt-subheader--enabled kt-subheader--transparent kt-aside--enabled kt-aside--fixed kt-aside--minimize kt-page--loading">
    ...
    <div class="kt-grid kt-grid--hor kt-grid--root">
        ...
        <div class="kt-content kt-grid__item kt-grid__item--fluid" id="kt_content">
            ...
            <!--begin::Form-->
                <form id="typeForm" action="{{ route('coverage.update',$coverage->id) }}"
                    ...
                    <div class="kt-portlet__body">
                        ...
                        <label for="example-text-input" class="col-3 col-form-label">Risk Name</label>
                            <div class="col-9">
                                <select class="form-control kt_selectpicker" id="riskTypeDropDown" title="Choose Risk"
                                        data-live-search="true"
                                        name="risk_type" required>
                                    @foreach($riskTypes as $risk)
                                        <option value="{{$risk->id}}" @if($coverage->risk == NULL)   @elseif($coverage->risk->name == $risk->name) selected @endif>{{$risk->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                         </div>
                        <div class="form-group row" >
                            ...
                            <div class="col-9">
                                <input  class="form-control" name="name" value="{{$coverage->name}}"  placeholder="Enter Coverage Name" title="Coverage name is required" required>
...
$(document).ready(function(){
    ...
    $("#riskTypeDropDown").change(function(){
        console.log("risk limit function");
        $.ajax({
...
$(document).ready(function(){
    var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
        $('#currentLimit').change(function() {
            ...
            console.log(coverageValue,risk_id);
