        ...
        <div class="kt-subheader kt-grid__item" id="kt_subheader">
            <div class="kt-subheader__main">
                ...
                <div class="kt-subheader__breadcrumbs">
                    ...
                    <span class="kt-subheader__breadcrumbs-link kt-subheader__breadcrumbs-link--active">Add New Contract</span>
                </div>
            </div>
        </div>
        <!-- end:: Subheader -->
    ...
    <!-- begin:: Content -->
        <div class="kt-content kt-grid__item kt-grid__item--fluid" id="kt_content">
            ...
            <div class="kt-portlet">
                <!--begin::Form-->
                <form id="accountCreate" action="{{ route('admin.storeNewClient') }}"
                      method="POST" enctype="multipart/form-data" class="kt-form">
                    <!-- CSRF Token -->
                    <input type="hidden" name="_token" value="{{ csrf_token() }}" />
                    <input type="hidden" name="returnBlade" value="{{ $returnBlade }}" />
                    <div class="kt-portlet__body">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-9">
                                <select class="form-control" name="BankCode" id="banks">
                                    ...
                                    @foreach($banks as $b)
                                        <option value="{{ $b->bank_number }}" @if($b->bank_number == $clientDetails['BankCode']) selected @endif>{{ $b->bank_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-9">
                                <select class="form-control" name="BranchCode" id="branches">
                                {{-- <option value="" disabled selected>Select Bank</option> --}}
                                    @foreach($branches as $br)
                                        {{-- @if($br->branch_id == $clientDetails['BranchCode']) --}}
                                            <option value="{{ $br->branch_id }}" @if($br->branch_id == $clientDetails['BranchCode']) selected @endif>{{ $br->name }}</option>
                                        {{-- @endif --}}
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-9">
                                <input  class="form-control" name="accountNumber" title="Please provide bank account number" placeholder="Please provide bank account number" value="{{$clientDetails['AccountNumber']}}">
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-9">
                                <select class="form-control" name="accountType" id="accountType">
                                    ...
                                    <option value="1" @if($clientDetails['AccountType'] == 1) selected @endif>Cheque</option>
                                    <option value="2" @if($clientDetails['AccountType'] == 2) selected @endif>Savings</option>
                                </select>
                            </div>
                        </div>
                        <div class="form-group row">
                        ...
                        <div class="form-group row firstCollectionDate" id="firstCollectionDateDiv">
                            <label  class="col-3 col-form-label">First Collection Date :</label>
                            <div class="col-9">
                                <input type="text" class="form-control kt_datepicker_1 dob" name="first_collection_date" id="first_collection_date" autocomplete="off"  placeholder="Select date" />
                                <span class="form-text text-muted"></span>
                            </div>
                        </div>
...
<script>
    ...
    $('#frequency').on('change',function(){
        ...
        if (freq == 3) {
            $('#firstCollectionDateDiv').hide();
            $('#first_premium_row').hide();
            $('#first_collection_date').attr('required', false);
            $('#first_premium').attr('required', false);
        } else {
            $('#firstCollectionDateDiv').show();
            $('#first_premium_row').show();
            $('#first_collection_date').attr('required', true);
            $('#first_premium').attr('required', true);
        }
    });
