...
<style>
/* AD Group KYC Admin Custom Styles */
.kt-widget1 {
    background: #fff;
    border-radius: 8px;
    padding: 20px;
        ...
        <div class="kt-subheader kt-grid__item" id="kt_subheader">
            <div class="kt-subheader__main">
                <h3 class="kt-subheader__title">
                    AD Group KYC Management Dashboard
                </h3>
                ...
                <div class="kt-subheader__breadcrumbs">
                    ...
                    <span class="kt-subheader__breadcrumbs-separator"></span>
                    <span class="kt-subheader__breadcrumbs-link kt-subheader__breadcrumbs-link--active">AD Group KYC Management</span>
                </div>
            </div>
                        ...
                        <table class="table table-striped table-bordered table-hover table-checkable" id="campaignsTable">
                            <thead>
                                <tr>
                                    ...
                                    <th style="color: #74788d; font-weight: 600;">Employer Group</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td style="color: #5867dd; font-weight: 600;">{{ $campaign->employer_group_id }}</td>
                                    ...
                                    <td>
                                        <div class="kt-datatable__cell">
                                            <span class="kt-datatable__cell-wrapper">
                                                <a href="{{ route('admin.ad-group-kyc.campaign.show', $campaign->id) }}"
                                ...
                                @empty
                                <tr>
                                    <td colspan="8" class="text-center" style="color: #74788d; padding: 40px;">No campaigns found</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
                        ...
                        <div class="list-group-item border-0 py-3">
                            <div class="d-flex align-items-start">
                                <div class="flex-shrink-0 me-3">
                                    <div class="bg-{{ $activity->activity_type === 'link_sent' ? 'primary' : ($activity->activity_type === 'link_opened' ? 'info' : ($activity->activity_type === 'notification_resent' ? 'warning' : 'success')) }} rounded-circle d-flex align-items-center justify-content-center"
                                         style="width: 40px; height: 40px;">
                                        <i class="fas fa-{{ $activity->activity_type === 'link_sent' ? 'paper-plane' : ($activity->activity_type === 'link_opened' ? 'eye' : ($activity->activity_type === 'notification_resent' ? 'redo' : 'check-circle')) }} text-white"></i>
                                    </div>
                                </div>
                                <div class="flex-grow-1">
                                    <div class="d-flex justify-content-between align-items-start mb-1">
                                        ...
                                        <small class="text-muted">
                                            ...
                                            {{ $activity->occurred_at->format('M d, H:i') }}
                                        </small>
                                    </div>
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Create New AD Group KYC Campaign</h5>
                <button type="button" class="close" data-dismiss="modal">
                    <span>&times;</span>
                </button>
            </div>
            <form action="{{ route('admin.ad-group-kyc.campaign.store') }}" method="POST">
                @csrf
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="employer_group_id">Employer Group</label>
                        <select class="form-control" id="employer_group_id" name="employer_group_id" required>
                            <option value="">Select Employer Group</option>
                            @foreach($employerGroups as $group)
                                <option value="{{ $group->employer_group_id }}">{{ $group->name }} ({{ $group->employer_group_id }})</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <form id="bulkNotificationsForm">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                    <button type="button" class="btn btn-primary" id="sendBulkNotificationsBtn">Send Notifications</button>
                </div>
            </form>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    console.log("Document ready - checking for sendBulkNotificationsBtn");
    console.log("Button exists:", $('#sendBulkNotificationsBtn').length > 0);

    // Test event delegation setup
    console.log("Setting up event delegation for #sendBulkNotificationsBtn");
    ...
    $('#campaignsTable').DataTable({
        ...
        "columnDefs": [
            ...
            { "className": "text-center", "targets": [0, 3, 4, 5] }, // Center align ID, Status, Links, Completion Rate
            { "width": "60px", "targets": [0] }, // ID column width
            { "width": "80px", "targets": [3] }, // Status column width
            { "width": "100px", "targets": [4] }, // Links column width
            { "width": "120px", "targets": [5] }, // Completion Rate column width
            { "width": "140px", "targets": [6] }, // Created column width
    ...
    // Bulk notifications button click handler (with fallback delegation)
    $(document).on('click', '#sendBulkNotificationsBtn', function(e) {
        e.preventDefault();
        alert("Main event delegation handler triggered!");
        ...
        const formData = new FormData($('#bulkNotificationsForm')[0]);
        const selectedLinks = $('input[name="link_ids[]"]:checked').map(function() {
            return this.value;
        }).get();

        console.log("Selected links:", selectedLinks);
        console.log("All link checkboxes:", $('input[name="link_ids[]"]').length);
        console.log("Checked link checkboxes:", $('input[name="link_ids[]"]:checked').length);
        ...
        console.log("All channel checkboxes:", $('input[name="channels[]"]').length);
        console.log("Checked channel checkboxes:", $('input[name="channels[]"]:checked').length);
        ...
        console.log("Campaign ID:", $('#bulk_campaign_id').val());

        // Send AJAX request
        $.ajax({
            url: '{{ route("admin.ad-group-kyc.campaign.send-links", ":id") }}'.replace(':id', $('#bulk_campaign_id').val()),
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function(response) {
                ...
                if (response.success) {
                    console.log("Success! Sending notifications...");
                    alert('Bulk notifications sent successfully! Success: ' + response.results.success_count + ', Failed: ' + response.results.failure_count);
...
function sendBulkNotifications(campaignId) {
    ...
    console.log("Campaign ID set to:", campaignId);
    console.log("Bulk campaign ID field value:", $('#bulk_campaign_id').val());

    // Load campaign links
    $.ajax({
        url: '{{ route("admin.ad-group-kyc.campaign.links-data", ":id") }}'.replace(':id', campaignId),
        type: 'GET',
        ...
        success: function(response) {
                    ...
                    const linkHtml = `
                        <div class="form-check">
                            ...
                            <label class="form-check-label" for="link_${link.id}">
                                ${link.employee_name || 'Unknown Employee'}
            ...
            // Debug: Check if button exists after modal is shown
            setTimeout(function() {
                console.log("Modal shown - checking for sendBulkNotificationsBtn");
                console.log("Button exists after modal show:", $('#sendBulkNotificationsBtn').length > 0);
                console.log("Button element:", $('#sendBulkNotificationsBtn')[0]);

                // Test if we can manually trigger click
                if ($('#sendBulkNotificationsBtn').length > 0) {
                    console.log("Button found - testing manual click trigger");
                    console.log("Button click events:", $._data($('#sendBulkNotificationsBtn')[0], 'events'));

                    // Add direct click handler as fallback
                    $('#sendBulkNotificationsBtn').off('click').on('click', function(e) {
                        e.preventDefault();
                        console.log("Direct click handler triggered after modal show");

                        // Implement the full functionality here
                        console.log("Starting form data collection...");
...
function sendEscalations(campaignId) {
    if (confirm('Send escalation notifications to all expired links in this campaign?')) {
        $.ajax({
            url: '{{ route("admin.ad-group-kyc.campaign.send-links", ":id") }}'.replace(':id', campaignId),
            type: 'POST',
            data: {
