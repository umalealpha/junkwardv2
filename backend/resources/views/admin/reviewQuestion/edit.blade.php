    ...
    <div class="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor">
        ...
        <div class="kt-content kt-grid__item kt-grid__item--fluid" id="kt_content">
            ...
            <div class="kt-portlet">
                <!--begin::Form-->
                <form id="regionCreate" action="{{ route('admin.review.question.updateQuestion',$question->id) }}"
                    ...
                    <div class="kt-portlet__body">
                        <div class="form-group row">
                            ...
                            <div class="col-6">
                            <input  class="form-control" name="question" value="{{ $question->question}}" title="Question is required" placeholder="Enter Question">
                                <span class="form-text text-muted"></span>
                            </div>
                        </div>   
                        <div class="form-group row {{ $errors->has('answer_type') ? 'has-error' : ''}}">
                            <label for="example-text-input" class="col-3 col-form-label">Answer Type :</label>    
                            <div class="col-6">
                                <select class="form-control kt_selectpicker" id="answer_type" title="Please choose element"
                                        data-live-search="true"
                                        name="answer_type" required>  
                                        @foreach($answer_types as $answer_type)
                                        <option value="{{$answer_type->id}}" @if($question->answer_type_id == null) @elseif($question->answer_type_id == $answer_type->id) selected @endif>{{$answer_type->type}}</option>
                                    @endforeach
                                </select>  
                                @if ($errors->has('answer_type'))
                                    <span class="col error">
                                        <p class="text text-danger">{{ $errors->first('answer_type') }}</p>
                                    </span>
                                @endif    
                            </div> 
                        </div>
                        <div class="form-group row">
                            <label for="example-text-input" class="col-3 col-form-label">Select Review Page</label> 
                            @foreach($reviewPages as $pages) 
                                <div class="col-sm-3 kt-checkbox-inline"> 
                                    <label class="kt-checkbox kt-checkbox--brand" for="{{$pages->name}}">{{ $pages->name }}  
                                    <input type="checkbox" class="input-group kt-group-checkable" @if($question->reviewpages != null) @foreach($question->reviewpages as $reviewpage) @if($reviewpage->id == $pages->id) checked= "checked" @endif @endforeach @endif  value="{{ $pages->id }}" id="{{ $pages->name }}" name="reviewPage[]" > 
                                        <span></span> 
                                    </label>
                                </div> 
                        ...
                        <div class="form-group row" style="margin-top: 1%">
                            ...
                            <div class="col-9">
                                <span class="kt-switch" >
                                    <label>
                                        <input id="switchValue" type="checkbox" @if($question->status) checked="checked" @endif name="status" value="1" onchange="statusMsg()">
