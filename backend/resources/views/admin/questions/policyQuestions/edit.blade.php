        ...
        <div class="kt-subheader kt-grid__item" id="kt_subheader">
            <div class="kt-subheader__main">
                ...
                <div class="kt-subheader__breadcrumbs">
                    ...
                    <span class="kt-subheader__breadcrumbs-separator"></span> <a href="" class="kt-subheader__breadcrumbs-link"> Dashboard </a> <span class="kt-subheader__breadcrumbs-separator"></span> <a href="" class="kt-subheader__breadcrumbs-link"> Policy Question</a> <span class="kt-subheader__breadcrumbs-separator"></span>
                ...
                <form id="productFactorMainEdit" action="{{ route('policy.question.update', $policyQuestion->id) }}"
                    ...
                    <div class="kt-portlet__body">
                        <div class="form-group row">
                            ...
                            <div class="col-9">
                                {!! Form::select('risk_type', $riskTypes, $policyQuestion->risk_type, ['class' => 'form-control']) !!}
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="example-text-input" class="col-3 col-form-label">Coverage</label>
                            <div class="col-9">
                                {!! Form::select('coverage', $coverages, $policyQuestion->coverage, ['class' => 'form-control']) !!}
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-9">
                                <input type="text" class="form-control" placeholder="Enter Question"
                                       name="question" value="{{ $policyQuestion->question }}">
                            ...
                            <div class="col-9">
                                <select class="form-control kt_selectpicker" id="productFactorType"
                                    ...
                                    @foreach($lookup as $record)
                                        <option value="{{$record->value}}" @if($policyQuestion->response_type == $record->value) selected @endif>{{$record->value}}</option>
                                    @endforeach
                                    </select>
                            </div>
                        </div>
                        ...
                        @foreach($pqValues as $pqValue)
                            ...
                            <div class="kt-repeater__close kt-repeater__close--align-right form-group" style="height: 0px;text-align: right;margin-bottom: 0px;position: relative;right: 10%">
                                <a href="{{route('policy.question.factorValueDelete',$factorValue->id)}}" class="btn btn-danger"> <i class="la la-close"></i> Close </a>
                            </div>
                            <div class="form-group row">
                                <label class="col-lg-3 col-form-label"></label>
                                <div class="col-lg-3">
                                    <input type="text" name="factor_type_values_names[{{$pqValue->id}}]" class="form-control" value="{{$pqValue->name}}" placeholder="Enter Value">
                                </div>
                                <div class="col-lg-3">
                                    <input type="number" name="factor_type_factor_names[{{$pqValue->id}}]" class="form-control" value="{{$pqValue->factor}}" placeholder="Enter Factor">
                                </div>
                            </div>
