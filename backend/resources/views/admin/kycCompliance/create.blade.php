            ...
            <div class="kt-content kt-grid__item kt-grid__item--fluid" id="kt_content">
                <div class="kt-portlet kt-portlet--mobile">
                    <div class="kt-portlet__body">
                        ...
                        @endif --}}
                        <form id="storeForm" action="{{ route('admin.kycCompliance.store') }}" method="POST"
                        enctype="multipart/form-data" class="kt-form">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-6">
                                    <input type="text" class="form-control @error('flow_id') is-invalid @enderror" name="flow_id"
                            ...
                            <table class="table table-striped table-bordered table-hover table-checkable" id="policy_table">
                                <thead>
                                    ...
                                    <tr id="tr_{{ $k }}">
                                        <th>
                                            <input class="form-control" type="hidden" name="data[{{ $k}}][field]" value="{{ $kyc_field->name }}">
                                                {{ $kyc_field->name }}
                                       </th>
                                        <td>
                                            <select name="data[{{ $k}}][check]" class="form-control @error('data.'.$k.'.check') is-invalid @enderror field" id="field_{{ $k }}" >

                                              {{-- <select name="data[{{ $k}}][check]" class="form-control field" id="field_{{ $k }}" > --}}
                                                <option value="">Select</option>
                                                <option @if( old('data.'. $k .'.check') == 1)  {{ 'selected'}} @endif value="1">Mandatory</option>
                                                <option @if( old('data.'. $k .'.check') == 2)  {{ 'selected'}} @endif value="2">Mandatory with Other field</option>
                                                <option @if( old('data.'. $k .'.check') == 3)  {{ 'selected'}} @endif value="3">Optional</option>
                                        ...
                                        <td>
                                            <select class="form-control @error('data.'.$k.'.other') is-invalid @enderror kycField" name="data[{{ $k}}][other]" id="kycField_{{ $k }}"
                                                ...
                                                @foreach ($kyc_fields as $kyc_field)
                                                    <option @if( old('data.'.$k.'other') == $kyc_field->slug)  {{ 'selected'}} @endif value="{{$kyc_field->slug}}"> {{old('data.'.$k.'other')}}  {{ $kyc_field->name }}</option>
                                                @endforeach
                                            </select>
                                                @error('data.'.$k.'.other')
                                                    <div class="text-danger">{{ $message }}</div>
                                        ...
                                        {{-- <td>
                                            <select class="form-control kycField" name="data[{{ $k}}][other]" id="kycField_{{ $k }}"
                                            style="display: none">
                                                <option value="">Select Mandatory field</option>
                                                @foreach ($kyc_fields as $kyc_field)
                                                    <option  value="{{$kyc_field->slug}}">{{ $kyc_field->name }}</option>
                                                @endforeach
                                            </select>
                                        </td> --}}
                                    </tr>
