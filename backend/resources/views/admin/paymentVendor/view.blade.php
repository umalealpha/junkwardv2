                        ...
                        <div class="kt-section">
                            <div class="kt-section__content">
                                <table class="table table-striped m-table">
                                    <tbody>
                                    <tr>
                                        <th>Payment Vendor Name:</th>
                                        @if($paymentVendor->vendorName != NULL)
                                            <td>{!! $paymentVendor->vendorName !!}</td>
                                        @else
                                            <td>Unavailable</td>
                                        @endif
                                    ...
                                    <tr>
                                        <th>Payment Vendor Label:</th>
                                        @if($paymentVendor->vendorLabel != null)
                                            <td>{!! $paymentVendor->vendorLabel !!}</td>
                                        @else
                                            <td>Unavailable</td>
                                        @endif
                                    </tr>
                                    <tr>
                                        <th>Payment Vendor Email:</th>
                                        @if($paymentVendor->email != null)
                                            <td>{!! $paymentVendor->email !!}</td>
                                        @else
                                            <td>Unavailable</td>
                                        @endif
                                    </tr>
                                    <tr>
                                        <th>Payment Vendor Telephone:</th>
                                        @if($paymentVendor->telephone != null)
                                            <td>{!! $paymentVendor->telephone !!}</td>
                                        @else
                                            <td>Unavailable</td>
                                        @endif
                                    </tr>
                                    <tr>
                                        <th>Payment Vendor Status:</th>

                                            @if($paymentVendor->status == 1)
                                                <td style="color:forestgreen"> Activated</td>
                                            @elseif($paymentVendor->status == 0)
                                                <td style="color:darkred">Deactivated</td>
                                            @endif
                    ...
                    <div class="kt-portlet__foot kt-portlet__foot--solid">
                        <div class="kt-form__actions">
                            <div class="row">
                                <div class="col-3"></div>
                                <div class="col-9">
                                    <a class="btn btn-secondary" href="{{ URL::to('admin/paymentVendor') }}" >Back</a>
                                </div>
                            </div>
                        </div>
                    </div>
