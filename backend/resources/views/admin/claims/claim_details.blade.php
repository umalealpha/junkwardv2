        ...
        <div class="kt-portlet__head-label col-lg-12">
            ...
            <div class="col-lg-2">
                @if($claims->status == 'Approved' || $claims->status == 'Rejected' || $claims->status == 'Closed')
                    @can('claim-edit')
                        <button class="btn btn-brand" style="float:right" id="editButton" onclick="setClaimToEdit()" hidden>Set to edit</button>
                    @endcan
                @elseif($claims->status == 'Pending' || $claims->status == 'Reopen')
                    @can('claim-edit')
                        <button class="btn btn-brand" style="float:right" id="editButton" onclick="setClaimToEdit()" >Set to edit</button>
                    @endcan
                @endif
            </div>
        </div>
