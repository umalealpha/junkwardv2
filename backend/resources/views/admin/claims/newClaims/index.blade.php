        ...
        <div class="kt-subheader kt-grid__item" id="kt_subheader">
            <div class="kt-subheader__main">
                ...
                <div class="kt-subheader__breadcrumbs">
                    ...
                    <span class="kt-subheader__breadcrumbs-link kt-subheader__breadcrumbs-link--active">New Claims</span>
                </div>
            </div>
    ...
    var KTDatatablesDataSourceAjaxServer = function() {
        ...
        var initTable1 = function() {
            table = $('#region_table').DataTable({
                ...
                order : [1, 'DESC'],
               ajax: {
                    url: '{!! route('admin.newclaims.data') !!}',
                    data: function (d) {
...
<script>
    ...
    <script>
         $("#region_table").on("click", "a.confirm-archive" , function(event) {
        event.preventDefault();
        var claim = $(this).attr('value');
        $.ajax({
            url: '{{ route('admin.newclaims.confirm-archive') }}',
            data: {
                "_token": "{{ csrf_token() }}",
                "id": claim
            },
            ...
            success: function(data) {
                $('#modal-content').append(append);
                var append = '<div class="modal-header"><h5 class="modal-title" id="exampleModalLabel">Archive Claim</h5>' +
                    '<button type="button" class="close" data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">&times;</span> </button> ' +
                    '</div> ' +
                    '<div class="modal-body"> ' +
                    '<p>'+data.body+'</p> ' +
                ...
                if(data.status == 'success'){
                    ...
                    var url = '{{ route("admin.newclaims.archive",":id") }}';
                    url = url.replace(':id', data.id);
