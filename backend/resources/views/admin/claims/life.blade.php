...
<div class="kt-portlet kt-portlet--height-fluid">
    ...
    <div class="kt-portlet__body">
        <div class="kt-widget-4">
            ...
            <form id="storeClaim" action="{{url('/claims/update/', [$claims->id])}}" method="POST" enctype="multipart/form-data">
                {{ method_field('PATCH') }}
                {{csrf_field()}}
                <div class="form-group row">
                        <label for="example-text-input" class="col-3 col-form-label">Date of Death</label>
                        <div class="col-9">
                            <input type="text" class="form-control kt_datepicker_1" name="date_of_death" autocomplete="off"  title="Date of death is required"  @if($life->date_of_death != NULL) value="{!! $life->date_of_death !!}" @endif>
                        </div>
                </div>
                <div class="form-group row">
                    <label for="example-text-input" class="col-3 col-form-label">Cause of Date</label>
                    <div class="col-9">
                        <select name="cause" class="form-control kt_selectpicker" data-live-search="true" title="Please select Cause of Death ">
                            @foreach($deathCauses as $deathCause)
                                <option value="{{ $deathCause->id }}" @if($life->cause_of_death == $deathCause->id) selected @endif>{{ $deathCause->value }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group row">
                        ...
                        <div class="col-2">
                            <div class="kt-avatar" id="death_certificate" style="float: left; clear: left;">
                                ...
                                @else
                                    <a href="{!! \AlphaDirect\Helper::getCloudFrontURL($life->certificate) !!}" target="_blank" download>
                                        <div class="kt-avatar__holder" style="background-image:url({!! AlphaDirect\Helper::getImageSrc($life->certificate) !!})"></div>
                                    </a>
                                @endif
                                <label class="kt-avatar__upload" data-toggle="kt-tooltip" title="Change Image">
                                    <i class="fa fa-pen"></i>
                                    <input type='file' class="death_certificate" name="death_certificate" <?php echo config('app.accept_attr'); ?> <?php echo config('app.accept_msg'); ?> />
                                </label>
                                <span class="kt-avatar__cancel" data-toggle="kt-tooltip" title="Cancel Image"> <i class="fa fa-times"></i> </span>
                            </div>
                        </div>
                        <div class="col-7">
                            <textarea class="form-control" name="description" placeholder="Add description" id="description" rows="3">{!! $life->description !!}</textarea>
                        </div>
                    </div>
...
{{--                        <div>--}}
{{--                            <button type="button" id="addBeneficiary" class="btn btn-brand">Add Beneficiaries</button>--}}
{{--                        </div>--}}
{{--                    @endif--}}
