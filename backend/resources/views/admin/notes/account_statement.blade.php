...
<head>
    ...
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
      ...
      <style>
        /* MIS account statement aligned to the COMG/DOMG (DomCom) statement:
           12px Calibri body, 11px table data, ~14px section headers,
           navy (#002060) text, line-height 1.45. The ledger logic below
           (running-balance recompute, Invoice/Payment/Refund split,
           credit-note reference lookup) mirrors the DomCom template. */
        body {
        ...
        <table style="width:100%;">
            <tr>
                <td width="60%" style="border: 0px" >
                    ...
                    <br>
                    <p style="font-family: Calibri!important;margin: 0px!important;color: #002060!important;">Alpha Direct Insurance Co. (Pty) Ltd.</p>
                    <p style="font-family: Calibri!important;margin: 0px!important;color: #002060!important;">Floor 2, Bar 2</p>
                    <p style="font-family: Calibri!important;margin: 0px!important;color: #002060!important;">Botswana Innovation Hub</p>
                    <p style="font-family: Calibri!important;margin: 0px!important;color: #002060!important;">Gaborone</p>
                </td>
            </tr>
        </table>
</header>
    ...
    <table style="width:100%;">
        <tr>
            <td width="100%" style="border: 0px">
                <p style="font-family: Calibri!important; margin: 0px!important;">Postal Address:</p>
                <p style="font-family: Calibri!important; margin: 0px!important;">P.O. Box 26ADC</p>
                <p style="font-family: Calibri!important; margin: 0px!important;">Gaborone</p>
                <p style="font-family: Calibri!important; margin: 0px!important;">Phone +267 392 8264 | Fax +267 392 8265</p>
                <p style="font-family: Calibri!important; margin: 0px!important;">accounts@alphadirect.co.bw | www.alphadirect.co.bw</p>
            </td>
        </tr>
    </table>
</div>
<div style="margin: 20px 0px;">
    <table style="table-layout: auto;width: 300px;">
        <tr>
            <td style="border: 0px">
                <p style="font-family: Calibri!important; margin: 0px!important;font-weight: 600!important;">Co. Regn. No. :</p>
                <p style="font-family: Calibri!important; margin: 0px!important;font-weight: 600!important;">VAT No. :</p>
            </td>
            <td style="border: 0px">
                <p style="font-family: Calibri!important; margin: 0px!important;font-weight: 600!important;">BW00000123907</p>
                <p style="font-family: Calibri!important; margin: 0px!important;font-weight: 600!important;">BW00000123907</p>
            </td>
        </tr>
    </table>
...
{{-- Defensive defaults: this template is rendered by the V2 PolicyCreateController
     (full data set, identical to the DomCom statement) AND by the legacy
     Admin\DocumentController::accountStatement (data/balance/customer/customer_profile
     only). The `??` fallbacks below keep the DomCom-derived logic safe for the
     minimal legacy caller. --}}
 @php
    $company          = $company ?? null;
    $policyNumber     = $policyNumber ?? (isset($policy) ? ($policy->policyNumber ?? '') : '');
    $opening_balance  = $opening_balance ?? 0;
    $sumInvoiceAmount = $sumInvoiceAmount ?? 0;
    $final_payment    = $final_payment ?? 0;
    $premium_freq     = $premium_freq ?? null;
 @endphp
 ...
 @foreach($data as $account_statement)
        ...
        if(\AlphaDirect\Services\AccountStatementService::canonicalTransType($account_statement->trans_type ?? '')!='Invoice'){
            ...
            $invoiceRefNumber =  \AlphaDirect\Ledger::where('id', $credit_note_file->invoice_id)->value('invoice_no');
                        ...
                        // AccountStatementService::rowAmount()). Refunds raise the
                        // amount owed on MIS and DomCom alike, which signedAmount()
                        // already encodes, so the MIS-specific branch is gone.
...
<div class="section">
    ...
    <table style="width:100%; margin-top:30px; border-collapse:collapse; border:1px solid #b8c1d4;">
        ...
        {{-- Credit notes and refunds move the Closing Balance, so the header
             cannot be reconciled by hand without them:
             Opening + Invoiced - Paid - Credit Notes + Refunds = Closing.
             Shown only when non-zero; null-coalesced because the legacy
             DocumentController caller does not pass them. --}}
        ...
        <table  style='border: 1px solid black;'>
            ...
            if(\AlphaDirect\Services\AccountStatementService::canonicalTransType($account_statement->trans_type ?? '')!='Invoice'){
                ...
                $invoiceRefNumber =  \AlphaDirect\Ledger::where('id', $credit_note_file->invoice_id)->value('invoice_no');
                            ...
                            // $rowAmt also feeds the amount cells below so the
                            // printed figures and the Running Balance agree.
            ...
            @if ($account_statement->credit != '-0.00')
                <tr style='border: 1px solid black;'>
                    ...
                    <td style='border: 1px solid black;'>
                        ...
                        @elseif ($premium_freq==2) Quarterly
                        @endif
                    </td>
    ...
    <div style="margin: 20px 0px 0px 0px;">
        <p style="font-family: Calibri!important; text-align: center; color: #002060!important;font-size: 16px;font-weight: 600!important; margin: 0px!important;">THANK YOU FOR YOUR BUSINESS!</p>
        <p style="font-family: Calibri!important; text-align: center;font-size: 14px; margin-top: 12px!important;font-weight: 600!important;">You just saved a lot of money by choosing Alpha Direct!</p>
        <p style="font-family: Calibri!important;margin: 10px 0px 6px 0px!important;">Bank: First National Bank (Botswana) Limited | Branch : Corporate | Branch Code : 282267 | SWIFT Code : FIRNBWGX | Account Number : 62403392335</p>
    </div>
