    ...
    <div style="position: relative; padding: 40px 0px;">
        <div class="inner-container clearfix" style="z-index: 11; margin: auto; ">
            ...
            <div>
                ...
                <h4 style="margin-bottom:30px;text-align: center;"> Thank you for your purchase!</h4>
            </div>
                        ...
                        <table class="table-responsive" style="border-collapse: collapse;width:100%; border:1px solid #c1c6d1;">
                            ...
                            <tr style="width: 100%;">
                               ...
                               <tr style="width: 100%;">
                                ...
                                <td style="width: 50%;">
                                    <p style="margin-bottom: 6px; margin: 0px;"></p>
                                </td>
                               </tr>
                              @if($product->id == 10)
                                <tr style="width: 100%;">
            ...
            @if(isset($product) && $product->id == 9 && isset($hospitalCoapplicants) && count($hospitalCoapplicants) > 0)
            <!-- ===== Hospital CashBack Co-Applicants ======= -->
            <h3 style="margin-top: 20px; margin-bottom: 10px; color: #383884!important;-webkit-print-color-adjust: exact;">Hospital CashBack Co-Applicants</h3>
            <table border="0" style="width: 100%; border:none; page-break-inside: avoid;">
                <tr>
                    <td style='border:none;padding: 0px;'>
                        <table class="table-responsive" style="border-collapse: collapse;width:100%; border:1px solid #c1c6d1;">
                            ...
                            @foreach($hospitalCoapplicants as $coapp)
                                @php
                                    $relationMap = [1 => 'Policy Holder', 2 => 'Spouse', 3 => 'Child'];
                                    $relation = $relationMap[$coapp->relation] ?? ucfirst($coapp->relation);
                                    $gender = $coapp->gender == 1 ? 'Male' : ($coapp->gender == 0 ? 'Female' : 'N/A');
                                    $dobValue = $coapp->dob ?? 'N/A';
                                    
                                    // Determine identity type and number
                                    $identityType = 'N/A';
                                    $identityNumber = 'N/A';
                                    if (!empty($coapp->omang)) {
                                        $identityType = 'Omang';
                                        $identityNumber = $coapp->omang;
                                    } elseif (!empty($coapp->passport)) {
                                        $identityType = 'Passport';
                                        $identityNumber = $coapp->passport;
                                    }
                                @endphp
                                <tr style="width: 100%;">
                                    <td>{{ $relation }}</td>
                                    <td>{{ trim(($coapp->first_name ?? '') . ' ' . ($coapp->middle_name ?? '') . ' ' . ($coapp->last_name ?? '')) }}</td>
                                    <td>{{ $dobValue }}</td>
                                    <td>{{ $gender }}</td>
                                    <td>{{ $identityType }}</td>
