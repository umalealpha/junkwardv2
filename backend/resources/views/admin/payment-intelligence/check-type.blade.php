    ...
    <div class="kt-container kt-container--fluid kt-grid__item kt-grid__item--fluid">
        ...
        <div class="row mb-3">
            <div class="col-12 d-flex justify-content-between align-items-center">
                <h4 class="mb-0"><i class="fas fa-search text-primary"></i> {{ $title }}</h4>
                <div>
                    ...
                    <a href="{{ route('admin.payment-intelligence.export', ['anomaly_type' => $anomalyType]) }}" class="btn btn-sm btn-outline-success"><i class="fas fa-download"></i> Export CSV</a>
                </div>
            </div>
        </div>
...
$(function() {
    var table = $('#anomaliesTable').DataTable({
        ...
        columns: [
            ...
            { data: 'description', render: function(d) {
                return '<span title="' + (d||'').replace(/"/g,'&quot;') + '">' + (d||'').substring(0,80) + (d && d.length>80 ? '...' : '') + '</span>';
            }},
            ...
            { data: 'payment_method', render: function(d) { return d || '-'; } },
            ...
            { data: null, orderable: false, render: function(data, type, row) {
                if (row.status === 'open') {
                    return '<form method="POST" action="/admin/payment-intelligence/anomalies/' + row.id + '/update-status" class="d-inline">@csrf<input type="hidden" name="action" value="acknowledge"><button type="submit" class="btn btn-xs btn-outline-info mr-1">Ack</button></form>' +
                    '<form method="POST" action="/admin/payment-intelligence/anomalies/' + row.id + '/update-status" class="d-inline">@csrf<input type="hidden" name="action" value="resolve"><input type="hidden" name="resolution_notes" value="Resolved"><button type="submit" class="btn btn-xs btn-outline-success mr-1">Resolve</button></form>' +
                    '<form method="POST" action="/admin/payment-intelligence/anomalies/' + row.id + '/update-status" class="d-inline">@csrf<input type="hidden" name="action" value="false_positive"><button type="submit" class="btn btn-xs btn-outline-secondary">FP</button></form>';
                }
                return '-';
            }}
        ]
    ...
    $('#filter_severity, #filter_status').change(function() { table.draw(); });
});
</script>
@endsection
