...
class Edit extends Component
{
    ...
    public $n_PrValidationRuleMaster_PK;
    ...
    public function mount($id){
        $this->n_PrValidationRuleMaster_PK = $id;
        $validation_rule =  ValidationRuleMaster::find($id);

        $validation_rule_detail = $validation_rule->detail()->get();
        foreach ($validation_rule_detail as $key => $value){
            $this->inputs[] = $key;
            $this->rule_for[$key] = $value->n_PrValidationCodeMasters_FK;
            $this->formula_expression[$key] = $value->s_FormulaExpression;
            $this->value[$key] = $value->s_CompareValue;
            $this->value_to[$key] = $value->s_CompareValueBetween;
        }
        $this->rule_code = $validation_rule->s_RuleCode;
        $this->product_id = $validation_rule->n_Product_FK;
        $this->screen_error_msg = $validation_rule->s_ScreenErrorMsg;

        $this->rule_description = $validation_rule->s_Description;
        $this->rule_apply_on = $validation_rule->s_RuleApplyOn;


        $this->can_rate_policy = $validation_rule->s_CanRate;
        $this->can_print_quote = $validation_rule->s_CanPrintQuote;
        $this->can_print_application = $validation_rule->s_CanPrintApp;
        $this->can_bind_application = $validation_rule->s_CanBindApp;
        $this->can_submit_un_bound_application = $validation_rule->s_CanUnBoundApp;
        $this->can_issue_policy = $validation_rule->s_CanIssue;
        $this->rule_start_date = $validation_rule->d_EffectiveDateFrom;
        $this->rule_end_date = $validation_rule->d_EffectiveDateTo;
        $this->rules_status = $validation_rule->s_RuleStatus=="ACTIVE" ? true : false;
    }
    ...
    public function submit(){
        ...
        foreach ($this->inputs as $index => $value){
            ...
            $this->rules["value_to.$index"] =  'required';
        }

        $validatedData = $this->validate();
        $validation_rule = ValidationRuleMaster::find($this->n_PrValidationRuleMaster_PK);
        ...
        // n_UpdatedUser + d_UpdatedDate are set automatically by the model's
        // boot() updating event. n_CreatedUser + d_CreatedDate are immutable
        // after creation and must not be overwritten on edit.
        if ($validation_rule->save()){
            $validation_rule->detail()->delete();
            ...
            activity('Validation Rule')
                ...
                ->log('Validation Rule Updated - '.$this->rule_code);
            session()->flash('success', "Validation Rule Updated Successfully");
    ...
    */
    ...
    {
        return view('v2.livewire.validation-rule.edit')->layout('layouts.app-v2');;
    }
}
