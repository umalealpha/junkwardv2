...
class SmsMessaging extends Controller
{
    ...
    public function NGeniusPayment($phoneNumber,$amount,$policy_number,$customer_firstName,$customer_lastName,$smsslug){
        try {
            $functionName = 'NGeniusPayment';
            $control =   SmsControls::where('function_name',$functionName)->first();
            if(isset($control) && $control->status == 1){
            $shortCodeArr = array('[POLICYNUMBER]','[CUSTOMER_NAME]','[PREMIUM]');
            $replacementArr =  array($policy_number,$customer_firstName.' '.$customer_lastName,$amount);
            $message = Sms::where('hook_slug', $smsslug)->first();
            $refinedMsg = strip_tags(str_replace($shortCodeArr, $replacementArr, str_replace('&nbsp;', '', $message->text)));
           // $response = InfobipSms::send('+267' . $phoneNumber, $refinedMsg);
            $response = event(new \AlphaDirect\Events\SendSms('+267' . $phoneNumber, $refinedMsg,['policyNumber' => $policy_number]));
    ...
    public function rekycVerification($smsTemplateId, $phoneNumber,$firstName,$rekycUrl,$rekycOtpExpiry)
    {
        $functionName = 'rekycVerification';
        $control =   SmsControls::where('function_name',$functionName)->first();
        if(isset($control) && $control->status == 1){
        $shortCodeArr = array('[FIRST_NAME]','[REKYC_URL]','[REKYC_OTP_EXPIRY]');
        $replacementArr = array($firstName,$rekycUrl,$rekycOtpExpiry);
        $message = Sms::where('id', $smsTemplateId)->first();
        $refinedMsg = strip_tags(str_replace($shortCodeArr, $replacementArr, str_replace('&nbsp;', '', $message->text)));
        $response = event(new \AlphaDirect\Events\SendSms(+919794613975,$refinedMsg,['policyNumber' =>'']));
    ...
    public function SendSMSEmailPolicyCancelled($phoneNumber,$firstName,$policyNumber)
    {
        ...
        $control =   SmsControls::where('function_name',$functionName)->first();
        if(isset($control) && $control->status == 1){
        $response = event(new \AlphaDirect\Events\SendSms('+267' . $phoneNumber, 'Dumelang ' . $firstName . ', Your policy '. $policyNumber .' has been cancelled successfully',['policyNumber' => $policyNumber]));
        return $response;
        }else{
            return true;
        }
    ...
    public function sendPaymentUrlGraphite(Request $request)
    {
        ...
        if ($paymentUrl->save()) { //save to DB
            try {
                ...
                // dd($customer);
                if ($oneTime != null) {
                    if($policy->product_id != 8 || $policy->product_id != 9){
...
//                $sms = $this->sendTsosologoSMS($sms_template_id, $data['cellphone'], $policyNumber, '', '', $addPyamentUrl->url,'');
    ...
    public function authenticateOTP(Request $request)
    {
        ...
        try {
            if ($cellphone != null && $otp_code != null) {
                ...
                // OTP" backdoor — twin of the one removed from
                // FrontendPay/CustomerController::authenticateOTP. It let any
                // otp_code that existed anywhere pass for any cellphone during a
                // single admin-set code's window (and `->first()->id` fataled on an
                // unknown cellphone). A wrong code now just fails below.
    ...
    public function sendPaymentFailedSMSForAllPaymentMethod($smsTemplateSlug,$c_name,$policyNumber,$cellphone,$amount){

        $functionName = 'sendPaymentFailedSMSForAllPaymentMethod';
        $control =   SmsControls::where('function_name',$functionName)->first();
        if(isset($control) && $control->status == 1){
        $shortCodeArr = array('[AMOUNT]','[POLICY_NUMBER]','[CUSTOMER_NAME]');
        $replacementArr = array($amount,$policyNumber,$c_name);
        $message = Sms::where('slug', $smsTemplateSlug)->first();
        $refinedMsg = strip_tags(str_replace($shortCodeArr, $replacementArr, str_replace('&nbsp;', '', $message->text)));
        $response = event(new \AlphaDirect\Events\SendSms('+267' .$cellphone,$refinedMsg,['policyNumber' => $policyNumber]));
        // $response = InfobipSms::send('+267' . $cellphone, $refinedMsg);
        // $response['policyNumber'] = $policyNumber;
    ...
    /**
     * MIS-only: recurring premium collection failed (DPO / RealPay).
     *
     * Takes an already-rendered body instead of a template id/slug: the caller
     * (MisRecurringPaymentFailureNotifier) needs the exact text it sent for the
     * failure ledger, and rendering it twice risks the two copies drifting.
     * The SmsControls gate stays here, where every other sender's gate lives.
     *
     * Unlike its siblings this returns FALSE (not true) when the control is
     * off, so the caller can tell "suppressed" from "dispatched".
     */
    public function sendMisRecurringPaymentFailedSMS($cellphone, $message, $policyNumber)
    {
        $functionName = 'sendMisRecurringPaymentFailedSMS';
        $control =   SmsControls::where('function_name',$functionName)->first();
        if(isset($control) && $control->status == 1){
            return event(new \AlphaDirect\Events\SendSms(
                '+267' . $cellphone,
                $message,
                ['policyNumber' => $policyNumber, 'hook' => \AlphaDirect\Services\MisRecurringPaymentFailureNotifier::TEMPLATE_SLUG]
            ));
        }else{
            return false;
        }
    ...
    public function sendRenewSmsWithNoPremiumChanged($phoneNumber)
    {
        $functionName = 'sendRenewSmsWithNoPremiumChanged';
        $control =   SmsControls::where('function_name',$functionName)->first();
        if(isset($control) && $control->status == 1){

        $response = event(new \AlphaDirect\Events\SendSms('+267' . $phoneNumber, 'If there is no change in premium we will inform customer that his policy will be renewed automatically.'));
        return true;
        }else{
            return true;
        }
    ...
    public function sendRenewSmsWithPremiumChanged($phoneNumber)
    {
        $functionName = 'sendRenewSmsWithPremiumChanged';
        $control =   SmsControls::where('function_name',$functionName)->first();
        if(isset($control) && $control->status == 1){

        $response = event(new \AlphaDirect\Events\SendSms('+267' . $phoneNumber, 'If there is change we will inform customer to either provide consent to us on email/sms link or his policy will be cancelled.'));
        return true;
        }else{
            return true;
        }
    ...
    public function sendRenewSmsForExpiredPolicy($phoneNumber)
    {
        $functionName = 'sendRenewSmsForExpiredPolicies';
        $control =   SmsControls::where('function_name',$functionName)->first();
        if(isset($control) && $control->status == 1){

        $response = event(new \AlphaDirect\Events\SendSms('+267' . $phoneNumber, 'Already Expired policies sms.'));
        return true;
        }else{
            return true;
        }
    ...
    public function sendPolicyExpiredSMS($phoneNumber)
    {
        $functionName = 'sendPolicyExpiredSms';
        $control =   SmsControls::where('function_name',$functionName)->first();
        if(isset($control) && $control->status == 1){

        $response = event(new \AlphaDirect\Events\SendSms('+267' . $phoneNumber, 'Your policy has been expired.'));
        return true;
        }else{
            return true;
        }
