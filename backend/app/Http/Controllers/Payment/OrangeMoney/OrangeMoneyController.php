...
class OrangeMoneyController extends Controller
{
    ...
    public function orangeCallback(Request $request)
    {
        ...
        try {
            ...
            // On a SUCCESSFUL Orange Money notification, also create the canonical
            // payment_transactions row so the ledger sweep can post it to the
            // Account Statement — previously orangeCallback wrote only
            // OrangeTransactions/transactions, leaving the payment invisible to
            // both ledger crons. Mirrors addCustomerTransactionOrange. Idempotent
            // via the referenceNumber (txnid) existence guard (callbacks may retry).
            if (strtoupper((string) $request->status) === 'SUCCESS'
                && $request->txnid != null
                && !PaymentTransaction::where('referenceNumber', $request->txnid)->exists()) {
                $today = Carbon::now()->format('Y-m-d');
                $payment = new PaymentTransaction();
                $payment->policyNumber            = $request->policyNumber;
                $payment->policy_id               = (isset($policy) && $policy) ? $policy->id : null;
                $payment->referenceNumber         = $request->txnid;
                $payment->amount                  = $request->amount;
                $payment->status                  = 'SUCCESS';
                $payment->paymentDate             = $today;
                $payment->new_payment_date        = $today;
                $payment->paymentMethod           = 'orangeMoney';
                $payment->is_ledger               = 0;
                $payment->numberOfInstalmentsPaid = 1;
                $payment->note                    = 'Orange Money callback';
                $payment->save();
            }
        } catch (Exception $re) {
        ...
        return response()->json(['code' => 200, 'message' => 'Orange callback processed'], 200);
    }

    public function getCSV(){
        if (Auth::user()->hasPermissionTo('orange_money_csv_upload'))
        {
