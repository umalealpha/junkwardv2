...
public function EmailSendVscCanceled($policyNumber){

    $policy = Policy::where('policyNumber', $policyNumber)->first();
     if($policy){
             $paytrx_graphite = \AlphaDirect\PaymentTransaction::where('paymentMethod','VCS')->where('policyNumber',$policy->policyNumber)
                               ->whereIn('status',['Success','SUCCESS','success',1])
                               ->whereBetween('created_at', [Carbon::parse('today')->subMonth(3)->format('Y-m-d')  , Carbon::parse('today')->subDay(1)->format('Y-m-d') . ' 23:59:59' ])
                               ->get();
             $paytrx_archive = \AlphaDirect\Models\PaymentTransactionArchive::where('paymentMethod','VCS')->where('policyNumber',$policy->policyNumber)
                               ->whereIn('status',['Success','SUCCESS','success',1])
                               ->whereBetween('created_at', [Carbon::parse('today')->subMonth(3)->format('Y-m-d')  , Carbon::parse('today')->subDay(1)->format('Y-m-d') . ' 23:59:59' ])
                               ->get();
        ...
        if(count($trxn) > 0){
                ...
                try{
                        if(isset($policy->customer) && $policy->customer->email != null){
                                ...
                                event(new \AlphaDirect\Events\SendMail($email,$emailTemplate->subject,"",$html,$data->attachment,['hook' => $data->hook]));
