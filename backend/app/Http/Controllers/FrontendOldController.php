...
class FrontendOldController extends Controller
{
    ...
    {
        $check = Productplan::where('product_id', $request->get('product_id'))->where('status', 1)->get(array('id', 'name', 'premium', 'sum_assured', 'product_id'));
        // Check if we are not trying to delete ourselves
    ...
    {
        return response()->json(['status' => 'success', 'premium' => 1000]);
        $check = Product::where('status', 1)->get(array('id', 'name'));
        // Check if we are not trying to delete ourselves
        if ($check->count() == null) {
            // Prepare the error message
