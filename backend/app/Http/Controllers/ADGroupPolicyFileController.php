...
// use Illuminate\Support\Facades\Cache; // already imported above in this controller
...
class ADGroupPolicyFileController extends Controller
{
    ...
    //         $filePath = 'adGroupFile/'. now()->format('Y_m_d_H_i_s').'_'.$name;
    //         Storage::disk('s3')->put($filePath, file_get_contents($file), 'public');
    //         $doc =  new ADGroupUploadedExcelFile();
    ...
    public function upload(Request $request)
    {
        set_time_limit(9999);
        $request->validate([
            'file' => 'required|mimes:xlsx,xls,csv,txt|max:16048',
            'type' => 'required|in:employer,membership'
        ]);
        ...
        if ($request->hasFile('file')) {
            ...
            $doc->file_name = $file->getClientOriginalName();
            $doc->file_path = $filePath;
            $doc->type = $request->input('type'); // employer or membership
            $doc->uploaded_by = auth()->user()->id ?? null;
            $doc->status = 'pending';
            $doc->remarks = 'Policy Create';
        ...
        return redirect()->route('admin.policy.excelCreatePolicies')->with('success', 'File uploaded successfully!');

    }

    public function processAdGroupUploads(Request $request)
    {
        try {
            // Initialize progress keys
            Cache::put('adgroup_progress_status', 'queued', 3600);
            Cache::put('adgroup_progress_processed', 0, 3600);
            Cache::put('adgroup_progress_total', 0, 3600);
            Cache::forget('adgroup_progress_message');

            $exitCode = ArtisanFacade::call('adgroup:process-uploads');
            $output = ArtisanFacade::output();

            return response()->json([
                'success' => $exitCode === 0,
                'message' => $exitCode === 0 ? 'Processing started and completed.' : 'Processing command exited with code '.$exitCode,
                'output' => $output,
            ]);
        } catch (\Throwable $e) {
            return response()->json([
    ...
    public function adGroupPendingCheck()
    {
        $pending = ADGroupUploadedExcelFile::where('status','pending')->where('type','membership')->count();
        return response()->json([
            'pending' => $pending,
        ]);
    }

    public function adGroupProgress()
    {
        return response()->json([
            'status' => Cache::get('adgroup_progress_status', 'idle'),
            'processed' => (int) Cache::get('adgroup_progress_processed', 0),
            'total' => (int) Cache::get('adgroup_progress_total', 0),
            'message' => Cache::get('adgroup_progress_message'),
            'upload' => Cache::get('adgroup_progress_upload'),
            'results' => Cache::get('adgroup_results')
        ]);
    }
    ...
    public function excelPoliciesCreate()
    {
        ...
        return view('admin.import.ad_group_policy_create');
        // } else {
    ...
    public function adGroupPolicy()
    {
        // if (auth::user()->hasPermissionTo('bonupolicy_list') || auth()->user()->hasRole('Super Admin')) {
            ...
            $agents = Policy::join('users', 'users.id', 'policies.agent_id')
                ...
                ->get();
            // Get employer groups for quote generation
            $employerGroups = EmployerGroup::select('id', 'name', 'employer_group_id')->get();
            
            // Show the page
            return view('admin.policy.adGroupIndex', compact('products', 'policies', 'agents', 'product_plans', 'employerGroups'));
        // } else {
        //     return Redirect::back()->with('error', 'Sorry! You do not have permission to access this page!');
        // }
    }
    public function adGroupPolicyData(Request $request)
    {
        ...
        // Total records
        $totalRecords = EmployerGroupPolicy::select('count(*) as allcount')->count();
        # DB::enableQueryLog();
        // Fetch records
        $records = EmployerGroupPolicy::join('policies', 'employer_group_policy.policyNumber', 'policies.policyNumber')->orderBy('policies.id', 'DESC')
    ...
    public function generateUserPoliciesPDF()
    {
        try {
            $userId = auth()->user()->id;
            
            // Get policies created by the logged-in user through AD Group uploads
            $policies = EmployerGroupPolicy::join('policies', 'employer_group_policy.policyNumber', 'policies.policyNumber')
                ->join('ad_group_uploaded_excel_files', function($join) use ($userId) {
                    $join->on('ad_group_uploaded_excel_files.id', '=', 'employer_group_policy.upload_id')
                         ->where('ad_group_uploaded_excel_files.uploaded_by', '=', $userId);
                })
                ->leftJoin('customer', 'customer.id', 'policies.customer_id')
                ->leftJoin('product_plans', 'product_plans.id', 'policies.plan_id')
                ->leftJoin('employer_groups', 'employer_groups.id', 'employer_group_policy.employer_group_id')
                ->select([
                    'customer.firstName',
                    'customer.middleName', 
                    'customer.lastName',
                ...
                ->whereNotIn('policies.product_id', [7, 8])
            ...
            // Generate PDF
            $pdf = PDF::loadView('admin.reports.user_policies_pdf', $data);
            
            return $pdf->download('user_policies_report_' . now()->format('Y_m_d_H_i_s') . '.pdf');
    ...
    {
        try {
            \Log::info('Quote data request received', [
                'employer_group_id' => $request->employer_group_id,
            ...
            // Apply filters
            if ($request->filled('employer_group_id')) {
                $query->where('employer_groups.employer_group_id', $request->employer_group_id);
            }
            ...
            $policies = $query->orderBy('policies.created_at', 'DESC')->paginate($perPage);

            \Log::info('Quote data query executed', [
                'policies_count' => $policies->total(),
            ...
            \Log::error('Error in getQuoteData', [
                'error' => $e->getMessage(),
    ...
    public function generateQuoteReport(Request $request)
    {
        try {
            ...
            $data = [
                ...
                'employer_group_name' => $policies->first()->employer_group_name ?? 'Unknown Employer Group'
            ];
