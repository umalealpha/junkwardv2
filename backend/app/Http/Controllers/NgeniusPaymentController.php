...
class NgeniusPaymentController extends Controller
{
       ...
       if(!isset($request->searchValue) || $request->searchValue == null){
        ...
        if(isset($request->agent_id) && $request->agent_id != null){
            ...
            if(isset($request->leadSource) && $request->leadSource == 'start.alphadirect.co.bw'){
                $updateCOntract->new_payment_method = "N-Genius_start";
            }else{
                $updateCOntract->new_payment_method = "N-Genius";
            }
    ...
    function identify()
    {
        ...
        $idUrl = env('NGENIUS_TOKEN_URL')."/identity/auth/access-token";
        $idHead = array(
    ...
    function pay($token, $outlet,$policyNumber,$leadSource)
    {
        ...
        $payUrl = env('NGENIUS_TOKEN_URL')."/transactions/outlets/$outlet/orders";
    ...
    public function NgeniusResponse(Request $request)
    {
        ...
        $RetrieveUrl = env('NGENIUS_RESPONSE_URL')."/transactions/outlets/$outlet/orders/$ref";
        $payHead = array("Authorization: Bearer ".$token, "Content-Type: application/vnd.ni-payment.v2+json", "Accept: application/vnd.ni-payment.v2+json");
        $payPost = null;
        ...
        if($payOutput->_embedded->payment[0]->state == "CAPTURED"){
            ...
            $trx->status = 1;
            $trx->save();
            // Re-enabled: on a CAPTURED result this redirect handler must land the
            // canonical payment_transactions row (was commented out, so the redirect
            // path never reached the ledger/statement). is_ledger=0 + new_payment_date
            // let the ledger sweep post it. Guarded by referenceNumber so it never
            // duplicates a row the server-to-server webhook may also create.
            if (!PaymentTransaction::where('referenceNumber', $payOutput->reference)->exists()) {
                ...
                $paymentTransaction->new_payment_date        = Carbon::now()->format('Y-m-d');
    ...
    public function NgeniusRecurring(Request $request)
    {
        ...
        if (isset($idData->access_token)) {
            ...
            if(isset( $payData->state ) && ($payData->state == "ACTIVE")){
                ...
                // Re-enabled: on an ACTIVE recurring result this handler must land the
                // canonical payment_transactions row (was commented out). is_ledger=0 +
                // new_payment_date let the ledger sweep post it. Guarded by
                // referenceNumber so it never duplicates the webhook's row.
                if (!PaymentTransaction::where('referenceNumber', $payData->reference)->exists()) {
                    $paymentTransaction                          = new PaymentTransaction();
                    $paymentTransaction->policyNumber            = $policy->policyNumber;
...
public function RecurringPay($token, $outletRecurring,$data)
    {
        ...
        CURLOPT_URL => env('NGENIUS_RESPONSE_URL').'/recurring-payment/outlets/'.$outletRecurring.'/orders',
    ...
    public function NgeniusRecurringGetdata(Request $request)
    {
                ...
                $curl = curl_init();
                curl_setopt_array($curl, array(
                CURLOPT_URL => env('NGENIUS_RESPONSE_URL').'/recurring-payment/outlets/'.$outletid.'/orders/'.$ref,
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => '',
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 60,
    ...
    public function NgeniusRecurringDeletedata3(Request $request)
    {
                $idData = $this->identify2();
               // Log::info(json_encode($idData));
            if(isset($idData->access_token)) {
                $token = $idData->access_token;
            }else{
                $token = null;
             return 2;
                ...
                $ngTrx = NgeniusTransection::where('policy_number',$request->policyNumber)->where('recurring_data','!=',null)->where('status',1)->orderby('id','desc')->first();
            if(isset($ngTrx) && $ngTrx->reference != null){
                $outletid = $ngTrx->outletId ;
                $ref = $ngTrx->reference;
                $curl = curl_init();
            ...
            if($info["http_code"] == 400 || $info["http_code"] == 404){
                ...
                $ngTrx->save();
                if (auth()->check()) {
                    $ngTrx->cancelled_by = auth()->user()->id;
                    $ngTrx->save();
                }
    ...
    {
        $pw = base64_decode(env('NGENIUS_PW'));
        $uid =  env('NGENIUS_USERNAME');
       
                        $curl = curl_init();

                curl_setopt_array($curl, array(
                CURLOPT_URL => env('NGENIUS_TOKEN_URL').'/identity/auth/access-token',
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => '',
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 0,
    ...
    public function ngeniuswebhook(Request $request)
    {
           ...
           if($request->eventName == "CAPTURED" || $request->eventName == "DECLINED"){
    ...
    public function ngeniuswebhookresponseVerify($ref,$orderType)
    {
       if(isset($ref)){
        ...
        if($payOutput->_embedded->payment[0]->state == "CAPTURED"){
            ...
            $trx->save();
            if(PaymentTransaction::where('referenceNumber',$payOutput->reference)->exists()){
                // Idempotent: this reference was already recorded (typically by the
                // redirect NgeniusResponse/NgeniusRecurring insert). Do NOT mint a
                // reference_<time> clone — that created a DUPLICATE ledger credit that
                // bypasses trans_ref dedup and the planned UNIQUE index. Reuse the
                // existing row, and still ensure the policy is activated (the redirect
                // path records the tx but does not activate).
                $paymentTransaction = PaymentTransaction::where('referenceNumber',$payOutput->reference)->orderBy('id','desc')->first();
                \Log::info('N-Genius CAPTURED webhook: payment_transactions already exists for '.$payOutput->reference.' — idempotent skip (no duplicate insert)');
                if($policy->status == 0){
                    $policy->policyActivatedDate = Carbon::now();
                    $policy->billingStartDate = Carbon::now();
                ...
                CustomerBanking::updateOrCreate(
                    ['policy_id' => $policy->id],
                    ['billing' => 'N-Genius', 'billingCell' => $policy->customer->cellphone, 'billingStartDate' => $policy->billingStartDate, 'billing_day' => $policy->billing_day]
                );
            }else{
            $paymentTransaction                          = new PaymentTransaction();
            $paymentTransaction->policyNumber            = $policy->policyNumber;
            ...
            $payment_status = 1;
           // $this->emailSms($policy,$amount,$payment_status,$ref);
            return 1;
            ...
            $paymentTransaction                          = new PaymentTransaction();
            $paymentTransaction->policyNumber            = $policy->policyNumber;
            $paymentTransaction->referenceNumber         = $payOutput->reference.'_'.time();
            $paymentTransaction->amount                  = ($payOutput->_embedded->payment[0]->amount->value) / 100;
            $paymentTransaction->status                  = "FAILED";
            $paymentTransaction->paymentDate             =  Carbon::now();
            $paymentTransaction->paymentMethod           = 'N-Genius';
            ...
            $paymentTransaction->TransactionToken        = null;
            $paymentTransaction->CompanyRef              = $payOutput->reference;
            $paymentTransaction->note = isset($payOutput->_embedded->payment[0]->authResponse->resultMessage) ? $payOutput->_embedded->payment[0]->authResponse->resultMessage : null;
            $paymentTransaction->reason = "Error Code is ".$payOutput->_embedded->payment[0]->authResponse->resultCode;
            $paymentTransaction->save();
            $amount = $paymentTransaction->amount;
            $payment_status = 0;
           // $this->emailSms($policy,$amount,$payment_status,$ref);
  ...
  public function emailSms($policy,$amount,$payment_status,$ref)
    {
        if(isset($policy->customer) && $policy->customer->email != null){
            ...
            $data->user_id     = null;
            if($payment_status == 1){
                $data->hook        = 'N-Genius_payment_success';
            }else{
                $data->hook        = 'N-Genius_payment_failed';
            }
            $data->customer_id = $policy->customer_id;
            $data->policy_id   = $policy->id;
            $data->totalcodes        = $amount;
            $data->link        = $ref.'_'.time();
         ...
         if(isset($policy->customer) && $policy->customer->cellphone != null){
            if($payment_status == 1){
                $smsslug        = 'N-Genius_payment_success';
            }else{
                $smsslug        = 'N-Genius_payment_failed';
            }
            ...
            $amount = $amount;
            // $sms = new SmsMessaging();
            // $sms->NGeniusPayment($phoneNumber,$amount,$policy_number,$customer_firstName,$customer_lastName,$smsslug);

          }
         return 1;

    }
    public function ngenius_get_trxn(){
        $idData = $this->identify();
        if (isset($idData->access_token)) {
        $token = $idData->access_token;
        }else{
        ...
        $ngTrx = NgeniusTransection::where('policy_number',"MIS2023020463")->orderby('id','desc')->first();
