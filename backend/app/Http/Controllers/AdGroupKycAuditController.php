...
use AlphaDirect\Services\AdGroupKycAuditService;
use Illuminate\Support\Facades\Validator;
use Carbon\Carbon;

class AdGroupKycAuditController extends Controller
{
    protected $auditService;

    public function __construct(AdGroupKycAuditService $auditService)
    {
    ...
    {
        $validator = Validator::make($request->all(), [
            'campaign_id' => 'nullable|exists:ad_group_kyc_campaigns,id',
            'start_date' => 'nullable|date',
            'end_date' => 'nullable|date|after:start_date'
        ]);
    ...
    public function exportAuditData(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $filename = 'ad_group_kyc_audit_export_' . Carbon::now()->format('Y-m-d_H-i-s') . '.' . $format;
