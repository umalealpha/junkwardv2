...
use AlphaDirect\Http\Controllers\Api\PolicyController;
...
//                }
    ...
    public function edit($id){
        ...
        $plan = null;
        if($document->product_id == 1){
            $plan = Productplan::where('product_id', $document->product_id)
            ->where('status', 1)
            ->get(array('id', 'slug', 'name', 'premium', 'sum_assured', 'product_id'));
        }


        return view('admin.documents.edit',compact('products','doc_types','document','plan'));
    }
    ...
    public function generatePolicyDocument($policyId)
    {
        //try {
            ...
            {
                $bundled_total_premium = 0;
                $bundles = PolicyBundled::where('policy_id', $policyId)->get(array('product_id','plan_name', 'premium', 'final_premium', 'subtotal'))->toArray();
                $bundled_total_premium = array_sum(array_column($bundles, 'premium'));
                $Bundledpremium = number_format(($policy->premium + $policy->bundled_discount_amount - $bundled_total_premium), 2, '.', ',');
            } else {
                $Bundledpremium = $policy->premium;
            }

            array_push($bundles, array('product_id'=>$policy->product_id,'plan_name'=>$policy->plan_id,'premium'=>$Bundledpremium));
            ...
            $vehicles = Vehicle::where('policy_id',$policyId)->get();
            $motorScheduleDetail = \DB::table('policy_coverages as pc')
                ...
                ->select('pcd.coverage_value as sum_insured', 'pcd.rate', 'pcd.calculated_value as premium')
                ->first();
            ...
            $data = [
                ...
                'vehicle' => $vehicle,
                'vehicles' => $vehicles,
                'motorScheduleDetail' => $motorScheduleDetail,
        ...
        if (isset($product) && $product->id == 9) {
            $data['hospitalCoapplicants'] = HospitalCashbackCoapplicants::where('policy_id', $policy->id)->get();
        }

            // BizSure / commercial (COMD) policies have no dedicated product_id
            // case in the switch below and previously fell through to the
            // `default:` branch, which renders the DOMESTIC MOTOR template
            // (mail_schedule.index) — a wrong document — with none of the
            // commercial relations loaded, so the schedule came out blank AND
            // mis-titled. Detect commercial policies here, load their relations
            // into $data, and let the `default:` branch render the dedicated
            // bizsure-commercial-schedule blade instead.
            $isCommercial = ($policy->leadSource === 'bizsure')
                || str_starts_with((string) $policy->policyNumber, 'COMD');
            $data['isCommercial'] = $isCommercial;

            if ($isCommercial) {
                // Parent covers, each with sub-covers (PolicyCoverageDetail),
                // extensions (PolicyExtentionDetails) and portable/specified
                // items (PolicySpecifiedItem). specifedItems() is withTrashed()
                // on the relation, so filter soft-deleted rows in the closure.
                $data['commercialCoverages'] = \AlphaDirect\Models\PolicyCoverage::where('policy_id', $policyId)
                    ->with([
                        'coverage',
                        'riskAddress.state',
                        'riskAddress.city',
                        'coverageDetail' => fn ($q) => $q->whereNull('deleted_at'),
                        'coverageDetail.coverage',
                        'extentionDetail' => fn ($q) => $q->whereNull('deleted_at'),
                        'extentionDetail.extention',
                        'specifedItems' => fn ($q) => $q->whereNull('deleted_at'),
                        'specifedItems.specifiedCoveragesItems',
                    ])
                    ->get();

                // Pty Ltd directors / shareholders (sole-prop policies = none).
                $data['commercialDirectors'] = \AlphaDirect\Models\PolicyDirector::where('policy_id', $policyId)->get();

                // Insured risk address(es) / premises.
                $data['commercialRiskAddresses'] = \AlphaDirect\Models\RiskAddress::where('policy_id', $policyId)
                    ->with(['state', 'city'])
                    ->get();

                // The 12 structured BizSure business fields already live on the
                // $policy row (business_name, occupation_type, annual_turnover,
                // years_in_business, number_of_employees, floor_area_sqm,
                // property_ownership, business_structure, company_reg_number,
                // tin_number, vat_number, postal_address) and reach the blade
                // via $data['policy'].
            }

            // Alpha Transit Cover (product 25): the risk detail lives in
            // mysql_system.atc_shipments, not in $data. Load it so the GIT
            // certificate blade can print sender/receiver/route/goods/cover.
            if ((int) $policy->product_id === 25) {
                $data['shipment'] = \DB::connection('mysql_system')->table('atc_shipments')
                    ->where('policy_id', $policyId)->orderByDesc('id')->first();
                $data['courierName'] = null;
                if ($data['shipment'] && !empty($data['shipment']->company_code) && $data['shipment']->company_code !== 'DIRECT') {
                    $data['courierName'] = \DB::connection('mysql_system')->table('atc_couriers')
                        ->where('company_code', $data['shipment']->company_code)->value('name');
                }
            }
            ...
            {
                if($policy->is_bundled == 1 && isset($bundle['product_id']) && isset($bundle['plan_name']) && $bundle['plan_name'] != null){
                   
                    $data['product'] =  Product::where('id',$bundle['product_id'])->first();
                  
                    $data['product_plan'] = Productplan::where('id',$bundle['plan_name'])->first();
                }
                $data['premium'] = $bundle['premium'];
                // Refresh co-applicants when generating Hospital CashBack schedule
                if (isset($bundle['product_id']) && $bundle['product_id'] == 9) {
                    $data['hospitalCoapplicants'] = HospitalCashbackCoapplicants::where('policy_id', $policy->id)->get();
                }
                switch ($bundle['product_id']) {
                    ...
                    case 9: //Hospital_Cashback
                        ...
                        $pdf = PDF::loadView('admin.notes.Schedule.index', $data);
                        break;
                    case 10: //Health In A Box
                        $path = 'PolicyDocument/'. $this->uuid() .'/'. $policy->policyNumber .'/Policy_Schedule_Health_In_A_Box.pdf';
                        $pdf = PDF::loadView('admin.notes.Schedule.index', $data);
                        break;
                    case 25: // Alpha Transit Cover (Goods-in-Transit) — certificate + schedule
                        $path = 'PolicyDocument/'. $this->uuid() .'/'. $policy->policyNumber .'/Policy_Schedule_Alpha_Transit_Cover.pdf';
                        $pdf = PDF::loadView('admin.notes.Schedule.git', $data);
                        break;
                    default:
                        if ($isCommercial) {
                            // BizSure / commercial (COMD) — dedicated commercial
                            // schedule blade with covers, extensions, specified
                            // items, risk address and directors. (Previously this
                            // fell through to the domestic-motor mail_schedule.index.)
                            $path = 'PolicyDocument/'. $this->uuid() .'/'. $policy->policyNumber .'/Policy_Schedule_Commercial.pdf';
                            $pdf = PDF::loadView('v2.livewire.pdf.bizsure-commercial-schedule', $data);
                        } else {
                            $path = 'PolicyDocument/'. $this->uuid() .'/'. $policy->policyNumber .'/Policy_Schedule.pdf';
                            $pdf = PDF::loadView('mail_schedule.index', $data);
                        }
    ...
    public function generatePolicyDocumentTerms($policyId,$term=null,$returnPath=false)
    {
        try {
            ...
            $vehicle = Vehicle::where('policy_id',$policyId)->first();
            $vehicles = Vehicle::where('policy_id',$policyId)->get();
            $motorScheduleDetail = \DB::table('policy_coverage as pc')
                ->join('policy_coverage_detail as pcd', 'pcd.policy_coverage_id', '=', 'pc.id')
                ->where('pc.policy_id', $policyId)
                ->whereIn('pc.coverage_id', [22, 27])
                ->whereNull('pcd.deleted_at')
    ...
    public function sendPolicyDocument($policyId,$sentBy='Agent',$isResend=false)
    {
        try {
            $policy = Policy::where('id',$policyId)->first();
            ...
            if(Documents::where('product_id', $policy->product_id)->where('status', 1)->where('plan_id', $policy->plan_id)->where('plan_id','!=',null)->exists()){
             $pid = Documents::where('product_id', $policy->product_id)->where('status', 1)
                                          ->where('plan_id', $policy->plan_id)->where('plan_id','!=',null)->get(['id']);
            if(count($pid) > 0){
                foreach($pid as $id){
                    $docsid[] = $id->id;
                }
            ...
            $policyCoverNote = PolicyCoverCancelNote::where('policy_id',$policyId)
                            ...
                            ->first(array('path'));
            $policySchedule = null;
            $policyAction = PolicyAction::where('policy_id',$policyId)->first();
            // if(isset($policyAction) && $policyAction->id != null){
            //     $policySchedule = GetPolicyDocuments::where('policy_id',$policyId)
            //     ->where('action_id',$policyAction->id)
            //     ->orderBy('id','DESC')
            //     ->first(array('doc_path'));
            // }
            ...
            // if(isset($policySchedule) && $policySchedule != null){
            //     if ($policySchedule->doc_path != null)
            //         array_push($attachments, $policySchedule->doc_path);
            // }

            if(isset($policyAction) && $policyAction->id != null){
                if($policyAction->schedule_doc != null){
                    array_push($attachments, $policyAction->schedule_doc);
                }
                if($policyAction->rate_doc != null){
                    array_push($attachments, $policyAction->rate_doc);
                }
                if($policyAction->quotation_doc != null){
                    array_push($attachments, $policyAction->quotation_doc);
                }
            }
            ...
            foreach ($attachments as $file) {
                ...
                $WhatsAppController->sendMessage($dataDocumentPolicy);
                }
            }
            // Policy Creation SMS — only on the actual create/first-send path,
            // not on a customer-triggered resend (MeController::resendDocuments
            // passes $isResend=true).
            if( $policy->agency_id != 26 && !$isResend ){
            $sms_status = event(new \AlphaDirect\Events\SendSms('+267' . $customer->cellphone, 'Hi' .$customer->firstName.' '. $customer->lastName. 'Sucessfully created your policy' . ' ' . $policy->policyNumber));
            }

            if($customer->email != null && $policy->agency_id != 26 ){
               $data = new \stdClass();
               //$data->user_id = $policy->id;
               $data->hook = 'policy_schedule';
               ...
               // $sent = Mail::to($customer->email)->send(new MailTemplate($data));
                $sent  = event(new \AlphaDirect\Events\SendMail($customer->email,$emailTemplate->subject,"",$html,$attachments,['policyNumber' => $policy->policyNumber,'hook' => $data->hook]));
        ...
        try{
            if($policyId != null){
                ...
                if($policy != null){
                    ...
                    $path = $this->generateQRCode($policy->policy_id);
                    ...
                    if($store != null){
                        ...
                        $ddelete  = new   DocumentDelete();
                        $ddelete->policy_id = $policyId;
                        $ddelete->document_name = $type.'_Note';
                        $ddelete->document = $store->path;
                        $ddelete->activity_by = auth()->check() ? auth()->user()->id : 'System';
                        $ddelete->save();
...
//                        );
                    ...
                    if($policy->email != null && $policy->agency_id != 26){
                        $data = new \stdClass();
                        $data->hook = $hook;
                        $data->customer_id = $policy->customer_id;
    ...
    // V2 Cancel Note (cover-note style) — product-agnostic version of generateCoverCancelNote;
    // no vehicle inner join so it works for non-motor products too
    public function generateV2PolicyCancelNote($policyId,$termId,$actionId){
        try{
            $policy = Policy::join('customer','customer.id','policies.customer_id')
                ...
                ->first(
                    array(
                        ...
                        'policies.product_id',
                        'policies.sum_assured as estimated_value',
                        'customer_banking.billing as billing',
                        'customer_banking.bankName',
                        'customer_banking.branchCode',
                        'customer.firstName',
                        'customer.middleName',
            ...
            $product = Product::where('id',$policy->product_id)->first(array('line_of_business'));
            $cancelAction = PolicyAction::where('id',$actionId)->where('policy_id',$policyId)->first();
            $cancelEffectiveFrom = (isset($cancelAction) && isset($cancelAction->effective_from)) ? Carbon::parse($cancelAction->effective_from)->format('d-m-Y') : null;

            if(isset($policy->billing) && $policy->billing == "RealPay" && $policy->bankName && $policy->branchCode){
                $bank = Banks::where('bank_number',$policy->bankName)->first(array('bank_name'));
                $branch = BankBranches::where('bank_id',$policy->bankName)
                    ->where('branch_id',$policy->branchCode)
                    ->first(array('name'));

                $policy['bankName'] = $bank->bank_name ?? $policy->bankName;
                $policy['branchCode'] = $branch->name ?? $policy->branchCode;
            }

            $policy['curr_date'] = Carbon::now()->format('d-m-Y');
            $this->generateQRCode($policy->policy_id);

            $array = [
                'policy' => $policy,
                'product' => $product->line_of_business ?? '',
                'cancelEffectiveFrom' => $cancelEffectiveFrom,
            ];

            $path = 'Policy/'. $this->uuid() .'/Cancel_'.$policy->policyNumber.'.pdf';
            libxml_use_internal_errors(true);
            $pdf = PDF::loadView('v2.livewire.pdf.policy_cancel_note_pdf', $array);
            Storage::disk('s3')->put($path, $pdf->output(), 'public');
            ...
            if($store != null){
                ...
                $ddelete = new DocumentDelete();
                $ddelete->policy_id = $policyId;
                $ddelete->document_name = 'Cancel_Note';
                $ddelete->document = $store->path;
                $ddelete->activity_by = auth()->check() ? auth()->user()->id : 'System';
                $ddelete->save();
                //*******End store deleted docs ***************
            }
            ...
            $p = Policy::where('id',$policyId)->first();
            activity('Create')
                ->performedOn($p)
                ->causedBy(User::where('id',auth()->user()->id)->first())
                ->log('Policy Cancel note has been created');

            return Redirect::back()->with('success', 'Policy Cancel note generated and stored successfully');
        }catch(\Exception $e){
            return Redirect::back()->with('error', $e->getMessage());
        }
    }
...
//        ));
                ...
                if($data->policy_id){
                    $policy = Policy::where('id',$data->policy_id)->first(array('verification_doc'));
                      ...
                      if($policy->verification_doc != null){
                        $ddelete  = new   DocumentDelete();
                        $ddelete->policy_id = $data->policy_id;
                        $ddelete->document_name = 'Information_Verification';
                        $ddelete->document = $policy->verification_doc;
                        $ddelete->activity_by = auth()->check() ? auth()->user()->id : 'System';
                        $ddelete->save();
                    }
                      //*******End tore deleted docs start ***************
    ...
    public function accountStatement($id) {
        ...
        $ledger = Ledger::where('policy_id', $id)
        ...
        $ledgerArchive = LedgerArchive::where('policy_id', $id)
        ->where(function ($query) {
            $query->whereNotNull('invoice_file')->orWhereNotNull('credit')->orWhere('trans_type', 'Refund');
        })->orderBy('id', 'asc')->orderBy('accounting_date', 'asc')->get();

        $merged = $ledgerArchive->merge($ledger)->sortby('accounting_date');
        $data = $merged->all();
        ...
        $balance = Ledger::where('policy_id', $id)->orderBy('system_date', 'DESC')->first(array('balance'));
        if ($balance != null) {
      ...
      public function accountStatementDomCom($id) {

        // $ledger = Ledger::where('policy_id', $id)->whereIn('trans_type',['Payment','Invoice','Credit Note'])
        //  ->whereNull('deleted_at')
        // ->orderBy('accounting_date','ASC')
        // ->get(array('invoice_no','trans_type','invoice_amount','id','credit','debit','balance','accounting_date','trans_ref','invoice_date','premium','accounting_date'));


        // Subquery: all valid payments that exist in payment_transactions
        $validPayments = DB::table('payment_transactions')
                        ->where('policy_id', $id)
                        ->whereNull('deleted_at')
                        ->where('status', 'SUCCESS')
                        ->whereNull('CompanyRef')
                        ->where('amount', '!=', 1)
                        ->pluck('referenceNumber');

        // dd($validPayments);

                // Ledger query excluding those payments
             $ledger = Ledger::where('policy_id', $id)
                        ->whereIn('trans_type', ['Payment', 'Invoice', 'Credit Note', 'Refund'])
                        ->whereNull('deleted_at')
                        ->where(function ($query) use ($validPayments) {

                            // Invoice & Credit Note → always
                            $query->whereIn('trans_type', ['Invoice', 'Credit Note'])

                            // Payment → always show, but only valid successful ones
                            ->orWhere(function ($q) use ($validPayments) {
                                $q->where('trans_type', 'Payment')
                                ->whereIn('trans_ref', $validPayments)
                                ->where('status', '!=', 'Reversed');
                            })

                            // Refund → show ONLY if linked to valid successful payment
                            ->orWhere(function ($q) use ($validPayments) {
        ...
        $merged = $ledgerArchive->merge($ledger)->sortby('accounting_date',SORT_REGULAR, false);
        $data = $merged->all();
        $final_invoiced_amount=0;
        $final_payment=0;
        foreach($data as $account_statement)
        {
          $final_invoiced_amount +=$account_statement->invoice_amount;
           $final_payment+=$account_statement->credit;
        }

        // added by snehal in 17-09-25 - final_invoiced_amount issue so added new code
        $sumInvoiceAmount = Ledger::where('policy_id',  $id)
        ->whereNull('deleted_at')
        ->where('trans_type', 'invoice')
        ->sum('invoice_amount');
        
        $policy = Policy::where('id',$id)->first(array('customer_id','policyNumber','premium_freq'));
        ...
        $customer_profile = CustomerProfile::where('customer_id',$policy['customer_id'])->first();
        // added by snehal on 30-07-2025
        $company = ($customer_profile && $customer_profile->company_id)
            ? Company::where('id', $customer_profile->company_id)->first(['id','name','VAT_registration_number','contact_person_number','primary_email'])
            : null;
      
        $balance = Ledger::where('policy_id', $id)->where('trans_type','invoice')->orderBy('system_date', 'DESC')->whereNull('deleted_at')->first();
       ...
       $opening_balance = Ledger::where('policy_id', $id)
    ->where('trans_type', 'invoice')
    ...
    ->sum('invoice_amount');
        ...
        // // $balance = Ledger::where('policy_id', $id)->where('trans_type', 'invoice')->orderBy('system_date', 'DESC')->whereNull('deleted_at')->first(array('balance'));
        // $query = Ledger::where('policy_id', $id)
        // ->where('trans_type', 'invoice')
        // ->whereNull('deleted_at')
        ...
        $array = [
            ...
            'final_invoiced_amount'=>$final_invoiced_amount,
            'sumInvoiceAmount'=>$sumInvoiceAmount,
            'final_payment'=>$final_payment,
        ];
        // view()->share('Customer',$array);
         $pdf = PDF::loadView('admin.notes.account_statement_domcom',$array);
        //return view('admin.notes.account_statement_domcom',$array);
        // download PDF file with download method
       return $pdf->download($customer->firstName.'_'.$customer->lastName.'_'.$policyNumber.'.pdf');
      }

      function calculateLedgerBalance($credit, $debit, $transType,$currentBalance)
      {
       // $currentBalance =0;
          // Default: subtract credit
          $currentBalance -= $credit;

          // Special case for Credit Note: subtract debit instead of credit
          if ($transType === 'Credit Note') {
          ...
          return (float) str_replace(',', '', number_format($currentBalance, 2));
      }

      public function accountNewStatement($id) {
        ...
        // view()->share('Customer',$array);
        $pdf = PDF::loadView('v2.livewire.policy.ledger.account_statement',$array);
        return view('v2.livewire.policy.ledger.account_statement',$array);
        // download PDF file with download method
        //return $pdf->download($customer->firstName.'_'.$customer->lastName.'_'.$policyNumber.'.pdf');
      }
    ...
    public function generateQRCode($policyID){
        try{
            ...
            File::makeDirectory($pathpublic, 0777, true, true);
            $path = public_path().'/images/qrcode'.$policyID.'.png';
