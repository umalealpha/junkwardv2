...
class AdGroupKycLink extends Model
{
    ...
    protected $casts = [
        ...
        'expires_at' => 'datetime',
        // Without this cast the OTP expiry check compares a raw string
        // against Carbon and silently misbehaves.
    ...
    /**
     * Mark link as sent. Reminders/escalations/resends call this on links
     * that may already be opened / otp_verified / completed — never regress
     * a progressed status back to 'sent'; only refresh the delivery stamps.
     */
    public function markAsSent($deliveryMethod = null, $deliveryReference = null)
    {
        ...
        if (in_array($this->status, ['pending', 'sent', 'failed'], true)) {
            $data['status'] = 'sent';
        }
        $this->update($data);
    }
