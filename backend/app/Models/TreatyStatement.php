...
class TreatyStatement extends Model
{
     ...
     *
     * THIS LIST AND CessionSource::MOTOR_GROUP_IDS ANSWER DIFFERENT QUESTIONS,
     * and the difference between them is deliberate on both sides.
     *
     * That constant is [13,14,15,28,29] and omits 30, MOTOR_TRADERS_COM_INT,
     * which RI-05 does list. It is not a defect: its own docblock records that
     * the five ids are carried across verbatim from graphiteBWV8, TmpTable and
     * the v2 API because the seam is a MOVE and not a rewrite — deriving them
     * would change which rows land in which branch and break the equivalence
     * the seam rests on. It reproduces what the legacy motor tab does, whether
     * or not the legacy tab is right.
     *
     * The stake is one row. On the book, group 30 carries a single cession
     * detail; group 29, which the constant DOES include, carries none at all.
     * Widening the constant would diverge from legacy across 407,000 rows to
     * capture one, so it stays as it is.
     *
     * This list is the statement's own question — which treaty reports a
     * cession — and RI-05 is its authority. So MOTOR_TRADERS_COM_INT is Motor
     * here and outside the legacy motor tab there, and both are correct.
     */
