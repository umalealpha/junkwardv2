...
class policyRenewalOperations extends Command
{
    ...
    public function handle()
    {
        ...
        try{
            // $policies = DB::select(DB::raw('select * from policies where status = 1 and product_id = 3 and and policyNumber = "MIS2022019177"'));
            ...
            if(count($policies) > 0){
                    foreach($policies as $key=>$policy) {
                        if (str_contains($policy->policyNumber,'MIS')) {
            ...
            if(count($data) > 0){
                foreach($data as $key=>$d){
                   ...
                   # $instalment = RealpayContractInstallments::where('clientNumber',$policy->policyNumber)->orderBy('id','desc')->first();
                    $renewal = PolicyRenewal::where('policyNumber',$d->policyNumber)->first();
 ...
 group by clientNumber;'));
            ...
            if(count($ref) > 0){
                foreach($ref as $key=>$r){
                    ...
                    if($cl){
                        $renewal = PolicyRenewal::where('policyNumber',$cl->clientNumber)->first();
