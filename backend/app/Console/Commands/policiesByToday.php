...
class policiesByToday extends Command
{
    ...
    public function handle()
    {
        ...
        $cron->save();
        /*#change*/
        $policies = Policy::whereBetween(\Illuminate\Support\Facades\DB::raw('date(policies.created_at)') , [Carbon::parse('today')
        ...
        $policy = Policy::join('products','products.id','policies.product_id')
            ->whereBetween(\Illuminate\Support\Facades\DB::raw('date(policies.created_at)') , [Carbon::parse('today')
            ->format('Y-m-d')  , Carbon::parse('today')
            ->format('Y-m-d') ])
        ...
        $activated = $policies->where('policies.status',1)->count();

        $instant = Policy::whereBetween(\Illuminate\Support\Facades\DB::raw('date(policies.created_at)') , [Carbon::parse('today')
            ->format('Y-m-d')  , Carbon::parse('today')
            ->format('Y-m-d') ])
            ->where('product_id','!=',3)
