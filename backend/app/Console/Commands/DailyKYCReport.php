...
class DailyKYCReport extends Command
{
    ...
    public function handle()
    {
        ...
        // customer_kyc.status was standardised 'Unapprove' -> 'rejected'
        // (migration 2026_08_07_000000_standardize_kyc_status_unapprove_to_rejected),
        // so the daily report must query the new value or it counts 0 unapproved.
        $unapproved = DB::select('call getKYCDataByAgent(?)',["rejected"]);
        ...
        $v_unapproved_all = Vehicle::join('policies','policies.id','vehicle.policy_id')
            ->whereBetween(\Illuminate\Support\Facades\DB::raw('date(vehicle.updated_at)') ,
