...
class updateCustomerKYCStatus extends Command
{
    ...
    public function handle()
    {
        ...
        $compliantData = KYC::where('compliance',1)
            ...
            ->get(['id','customer_id','compliance','status','omangExpiry','licenseExpiry','passportExpiry']);
        ...
        if(count($compliantData) > 0){
            foreach($compliantData as $key=>$cd){
                try{
                    ...
                    // nothing to chase docs for once the policy is dead.
        ...
        $report = [
            ...
            'title'    => 'Customer kyc status updated List'
        ];
