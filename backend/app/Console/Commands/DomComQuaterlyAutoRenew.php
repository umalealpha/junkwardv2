...
class DomComQuaterlyAutoRenew extends Command
{
     ...
     * @var string
     */
    protected $signature = 'DomComQuaterlyAutoRenew:cron {--policy= : Restrict the run to a single policy (id or policyNumber) — for local/testing. Omit to run the full batch as the scheduler does.}';
     ...
     * @var string
     */
    protected $description = 'Dom/COm Quaterly Auto Renew';
    ...
    public function handle()
    {
        ...
        $cron->save();
        Log::info("DomComQuaterlyAutoRenew Cron is working fine!");
    ...
    private function doHandle(CronStatus $cron)
    {
        ...
        $policies = DB::table('policies as p')
            ...
            ->where('p.premium_freq', 5)          // Quarterly
            // ->where('p.status', 1)                // Active
            ->whereNotIn('p.status', [2])          // Not cancelled
            ->whereDate('p.created_at', '>=', '2024-07-01')

            /* Must have latest ISSUED valid transaction */
            ->whereExists(function ($query) {
                $query->select(DB::raw(1))
                    ->from('policy_actions as pa')
                    ->whereColumn('pa.policy_id', 'p.id')
                    ->where('pa.status', 'ISSUED')
                    ->whereIn('pa.transaction_type', [
                        'NEWBUSINESS',
                        'RENEW',
                        'REISSUE',
                        'REINSTATE'
            ...
            //->whereIn('p.id', $ids) // optional filter for testing
            // Optional single-policy filter (local / testing). When --policy is
            // omitted the run is unchanged — all eligible quarterly policies,
            // exactly as the scheduler invokes it. Eligibility gates above still
            // apply, so a policy that isn't actually due won't renew.
            ...
            ->orderBy('p.id')
            ->select('p.id', 'p.policyNumber', 'p.product_id', 'p.status')
            ->get();
        ...
        foreach ($policies as $policy) {
            ...
            /* 1️⃣ Latest action safety (CANCEL / LAPSED stop) */
            ...
            /* 4️⃣ Cursor starts from NEWBUSINESS end — unless an anniversary
               renewal was ISSUED: when UW changes the period of insurance on
               the anniversary, all later RENEWs must continue from the
               anniversary dates, not the original NEWBUSINESS schedule. */
            $cursor = Carbon::parse($nbAction->effective_to);
            $anniversaryAnchor = $policyStartDate->copy();
            if ($latestIssuedAnniversary) {
                $cursor = Carbon::parse($latestIssuedAnniversary->effective_to);
                $anniversaryAnchor = Carbon::parse($latestIssuedAnniversary->effective_from);
            }
            ...
            /* 5️⃣ Hard stop date. A pending anniversary QUOTE ends the schedule:
               nothing may be created on/after its start date until UW issues it. */
            $hardStopDate = $pendingAnniversaryQuote
                ? Carbon::parse($pendingAnniversaryQuote->effective_from)->subDay()
                : Carbon::today();

            /* 6️⃣ Quarterly cycle loop (3-month increments from the schedule anchor)
               ...
               quarter, instead of permanently drifting to the clamped day. */
            $originalDay = (int) $anniversaryAnchor->day;
            $anchorMonth = (int) $anniversaryAnchor->month;
            $anchorYear  = (int) $anniversaryAnchor->year;
            ...
            /* A REINSTATE / REISSUE resets the coverage cadence: everything
               ...
               that a later quarterly reinstate replaced still spans its whole
               year, so it "overlaps" every quarter and blocks the pending
               renewal forever. The next renewal follows the last live period
               (the reinstate/renew chain), not the superseded annual row. Null
               when the policy was never reinstated → overlap guard unchanged. */
            ...
            for ($offset = 3; $offset <= 300; $offset += 3) {
                ...
                // 🔹 Next quarter — cycleStart = anchor + $offset months, day clamped
                ...
                // cycleEnd = next quarter start − 1 day (same clamp)
                $totalMonthsEnd = $totalMonths + 3;
                ...
                /* Premium / replication SOURCE = the ISSUED action with the
                   ...
                   the true latest period (e.g. the anniversary). */
                ...
                /* 7️⃣ Anniversary window check (1 year ± 3 months) — anchored on
                   the issued anniversary when present so a date change on the
                   anniversary moves the next anniversary with it. */
                $annivStart = $anniversaryAnchor->copy()->addYear();
                $annivEnd = $annivStart->copy()->addMonths(3)->subDay();

                if ($cycleStart->between($annivStart, $annivEnd)) {

                    // Create ANNIVERSARY QUOTE and STOP
                    ...
                    Log::info("Policy {$policy->id}: Anniversary quote created, stopping.");

                    break;
                }

                /* 8️⃣ Create quarterly RENEW (ISSUED) */
                $action = $this->createPolicyActionFromPrevious(
                    $sourceAction,
                    'RENEW',
                    'ISSUED',
                ...
                // VERBATIM — a quarterly RENEW is a pure replica of the source
                ...
                Log::info("Policy {$policy->id}: Quarterly renewal {$effectiveFrom} → {$effectiveTo}");
            }

            // [GAP-FILL] Patch any pending quarters the cadence loop above could
            // not reach — an orphan quarter BEFORE an off-cadence anniversary and
            // the trailing quarters AFTER it (see backfillPendingPeriods).
            $this->backfillPendingPeriods($policy, $nbAction, 3);
        }
     ...
     * (a) reach back and fill a quarter orphaned BEFORE an off-cadence
     * anniversary, nor (b) resume cleanly AFTER an anniversary whose UW-set day
     * doesn't match the policy's renewal day. This pass walks the policy's actual
     * covered timeline and creates an ISSUED RENEW for every uncovered slot up to
     * the hard stop, clipping each slot to its neighbouring period so it can
     * never overlap one.
     *
     ...
     * cadence loop's job; this pass only ever writes ordinary RENEWs to patch
     * holes around an already-established anniversary.
     *
    ...
    private function backfillPendingPeriods($policy, $nbAction, int $intervalMonths): void
    {
        ...
        // Trailing fill stops the day before a pending anniversary QUOTE, else today.
        // EARLIEST pending quote — the first one awaiting UW closes the schedule,
        // so a later pending quote must not raise the ceiling.
                    ...
                    foreach ($records as $policy) {
                        ...
                        foreach ($policyAction as $policyActions) {
                            ...
                            $check_invoice_exists = Ledger::where('policy_id', $policy_id)
                                ...
                                ->whereNull('deleted_at')
                                ->count();
                            if ($check_invoice_exists == 0 && ($policy->premium_freq == 5 || $policy->premium_freq == 2 || $policy->premium_freq == 3)) {
