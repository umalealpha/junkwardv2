...
class createInvoiceAutoRenewPolicyMonthly extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'invoiceMonthly:domcom';
     ...
     * @var string
     */
    protected $description = 'Invoices DomCom Monthly Auto Renew';
            ...
            //whereDay('billingStartDate', $today) 
                ...
                foreach($policies as $records) {
                    foreach($records as $policy)
                    {
                        ...
                        $policyAction = PolicyAction::where('policy_id', $policy_id)->where('status', 'ISSUED')->get();  
                        ...
                        if($check_invoice_exists ==0 && ($policy->premium_freq == 1 || $policy->premium_freq == 2 || $policy->premium_freq == 3 || $policy->premium_freq == 5)){
                            $ledger = Ledger::where('policy_id', $policy_id)->where('trans_type', 'Invoice')->orderBy('id', 'DESC')->where('trans_type', 'Invoice')->first(array('invoice_no', 'banking_id', 'invoice_date'));
                            ...
                            $balance = 0;
                             $balance = Ledger::where('policy_id', $policy->id)->where('trans_type', 'Invoice')->orderBy('id', 'DESC')->first(array('balance'));
                            if ($balance != null) {
