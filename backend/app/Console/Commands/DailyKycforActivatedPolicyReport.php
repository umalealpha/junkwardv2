...
class DailyKycforActivatedPolicyReport extends Command
{
    ...
    public function handle()
    {
        ...
        $policies=KYC::join('policies','customer_kyc.customer_id','=','policies.customer_id')
            ...
            ->whereBetween(\Illuminate\Support\Facades\DB::raw('date(customer_kyc.updated_at)') , [Carbon::parse('today')->subDay(1)
                ->format('Y-m-d')  , Carbon::parse('today')->subDay(1)
                ->format('Y-m-d') ])
                ...
                ->where(function ($query2) {
                    ...
                    ->OrwhereBetween(\Illuminate\Support\Facades\DB::raw('date(customer_kyc.approved_date)') , [Carbon::parse('today')->subDay(1)
                    ->format('Y-m-d')  , Carbon::parse('today')->subDay(1)
                    ->format('Y-m-d') ]);
                  })
                ->get(array(
        ...
        foreach($total as $key => $p)
         {
            ...
            else
            {
                if(($p->omang != null && $p->omangBack !=null )|| ($p->passport !=null))
                {
                    // $product was previously left undefined when product_id
                    // was null OR Product::find() returned null (deleted
                    // product). Then `$product->name` threw
                    // "Attempt to read property 'name' on null" and crashed
                    // the entire daily-KYC report mid-loop. Defaulting to
                    // null keeps the loop running and surfaces the missing
                    // product as a blank cell in the report instead of a
                    // hard failure.
                    $product = ($p->product_id != null)
                        ? Product::find($p->product_id)
                        : null;
                    array_push($total_count,['policyNumber'=>$p->policyNumber,'Customer'=>$p->firstName.' '.$p->lastName ,'Product'=>$product->name ?? null,'policy_status'=>$p->status,'compliance'=>$p->compliance,'Kyc_status'=>$p->kyc_status]);
                }
            }
        }
        ...
        ->where('policies.status',1)
        ->orderby('customer_kyc.id','desc')
        ->where(\Illuminate\Support\Facades\DB::raw('date(customer_kyc.updated_at)'),'<',Carbon::parse('today')->subDay(1)->format('Y-m-d'))
        ->where(function ($query) {
            $query->where('customer_kyc.status','!=' ,'Unchecked')
            ->orWhere('customer_kyc.status','Renew');
          })
        ...
        ->orderby('customer_kyc.id','desc')
        ->whereBetween(\Illuminate\Support\Facades\DB::raw('date(customer_kyc.updated_at)') , [Carbon::parse('today')->subDay(1)
        ->format('Y-m-d')  , Carbon::parse('today')->subDay(2)
        ->format('Y-m-d') ])
        ->where(function ($query) {
            $query->where('customer_kyc.status','!=' ,'Unchecked')
            ->orWhere('customer_kyc.status','Renew');
