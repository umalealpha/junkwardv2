...
use AlphaDirect\Models\LedgerArchive as Ledger;
use AlphaDirect\Models\PaymentTransactionArchive as PaymentTransaction;
...
use AlphaDirect\Models\SubledgerArchive as SubLedger;
...
class PolicyLedgerArchive extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'policyledgerarchive:cron';
     ...
     * @var string
     */
    protected $description = 'Policy ledger archive';
    ...
    {
        $cron = new CronStatus();
        $cron->name = "policyledgerarchive:cron";
        $cron->start = \Carbon\Carbon::now();
        $cron->save();
        ini_set('max_execution_time', 0);
        try{

            $policies = Policy::where('id', 10930)
            ...
            foreach($policies as $records)
            {
                ...
                foreach($records as $policy)
                {
                   ...
                   //For Motor Comp First Invoice on Policy Activation dates
                    ...
                    {
                        //Check if Policy is renewed
                        //$diff_in_years = $now->format('Y') - Carbon::parse($created_date)->format('Y');
                        //if($diff_in_years > 1 && $ledger_count > 0)
                        //{
                            ...
                            if($renew != NULL && $renew->is_renewed == 1)
                            {
                                ...
                                $latest_term = PolicyTerm::where('policy_id', $policy->id)->orderBy('id', 'desc')->first(array('term_start_date'));
                                if($latest_term != NULL)
...
// dd($policy->premium.'-'.$created_date.'-'.$diff_in_months.'-'.$first_invoice);
                    for($i = 0; $i < 3; $i++)
                    {
                        ...
                        $archiveDate = Carbon::parse($created_date)->lt('2022-07-01');
                        //echo '//'.$created_date.'--'.$now.'--'.$created_date->lte($now).'--'.$cancelled_status.'//';
                        if(Carbon::parse($created_date)->lte($now) && $cancelled_status && $check_invoice_exists == 0 && $archiveDate)
                        {
                            //-------------------------------PREMIUM--------------------------------------
            ...
            dd('Policy Done');
            //For Regenerate
            $transactions = PaymentTransaction::where('policyNumber','MIS2021010265')->where('is_ledger', 0)->where('amount', '!=', 1)->where('is_refund', 0)->get();
            foreach($transactions as $transaction)
            {
                ...
                if($policy != NULL && $policy->status == 1)
                {
                    ...
                    $subRecord['policy_id'] = $policy->id;
                    $subRecord['claim_id'] = NULL;
                    $subRecord['banking_id'] = $banking->id;
                    $subRecord['account_name'] = 'Accounts Receivable A/C';
            ...
            //dd('Done');
            $refunds = PaymentTransaction::where('policyNumber','MIS2021010265')->where('is_ledger', 0)->where('is_refund', 1)->get();
            foreach($refunds as $refund)
            {
                ...
                if($policy != NULL)
                {
                    ...
                    if($ledger != NULL)
                    {
                        ...
                        $record['trans_type'] = 'Refund';
                        $record['amount_type'] = NULL;
                        $record['trans_ref'] = strtoupper($refund->reference_number);
                        $record['orig_trans'] = strtoupper($refund->reference_number);
                        $record['unallocated'] = NULL;
                        ...
                        $subRecord['trans_type'] = 'Cash Refund';
                        $subRecord['trans_ref'] = strtoupper($refund->reference_number);
