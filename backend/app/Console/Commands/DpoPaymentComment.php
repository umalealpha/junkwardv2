    ...
    public function handle()
    {
        ...
        if(count($policies) > 0)
        {
            foreach($policies as $policy)
            {
                ...
                $policyChk         = Policy::where('policyNumber',$policy->policyNumber)->first();
                        ...
                        $data = [
                            ...
                            if(isset($policy->subscription_token))
                            {
                                    ...
                                    $data['customerToken']            = $policy->customer_token;
                                          ...
                                          $policy->status             = 1;         
                                          }else if($policyChk->product_id==3 || $policyChk->product_id==5){
                                            $chk=PolicyController::checkMotorpolicyStatus($policyChk->product_id,$policy->id,2);
                                            $policy->status             =  $chk; 
                                          }                                                                           //in progress
