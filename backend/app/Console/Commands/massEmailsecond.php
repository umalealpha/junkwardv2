...
class massEmailSecond extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'massEmailSecond:cron';
     ...
     * @var string
     */
    protected $description = 'Mass Email Second';
    ...
    {
        $cron = new CronStatus();
        $cron->name = "massEmailSecond:cron";
      ...
      //  if(env('APP_STATUS') == 'Production') {
           ...
           $gfs = GFSEmails::whereNotNull('Email_Address')->where('id','>=','1981')->get();
            foreach($gfs as $d){
                ...
                $data = new \stdClass();
                $data->user_id = null;
                $data->hook = 'mass_crisis_email2';
                $data->customer_id = null;
                $data->attachment = $attachments;
                $emailTemplate = EmailBroadcasting::where('hook_slug', $data->hook)->first(array('subject'));
                $markdown = new MailTemplate($data);
                $html = $markdown->render('Mail.mailTemplate',['data'=>$data]);
                event(new \AlphaDirect\Events\SendMail(strtolower(trim($d->Email_Address)),$emailTemplate->subject,"",$html,$attachments,['hook' => $data->hook]));
            ...
            foreach($policies  as $policy){
                ...
                $markdown = new MailTemplate($data);
                $html = $markdown->render('Mail.mailTemplate',['data'=>$data]);
                event(new \AlphaDirect\Events\SendMail($policy->email,$emailTemplate->subject,"",$html,$attachments,['hook' => $data->hook]));
               // $sent = \Illuminate\Support\Facades\Mail::to($policy->email)->send(new MailTemplate($data));

                $log = new VATMemoLog();
                $log->document = 'mass_crisis_email2';
