...
class OrangeScheduledTransactionPayment extends Command
{
    ...
    public function handle()
    {
        ...
        //dd($date);
        $scheduledTransaction = ScheduleTransaction::where('payment_method','orange')->whereDate('execution_date', $date)->where('status','=','0')->get();
        # dd($scheduledTransaction);
        $this->info('Running...');
        if (isset($scheduledTransaction)) {
            foreach ($scheduledTransaction as $key => $transaction) {

                $this->info('Running scheduled...'. $transaction->policy_number);
                $orangeCon = new PaymentController();
                $token = $orangeCon->orangeAccess();
                Log::info('Cron Started to execute orange order');
                $policy = Policy::where('policyNumber', $transaction->policy_number)->first();
                $curl = curl_init();
                ...
                curl_setopt_array($curl, array(
                    ...
                    // "executionDate": "'. Carbon::today()->addDays(2)->format('Y-m-d') .'"
                    CURLOPT_POSTFIELDS => '[
                        {
