...
class ScheduleClearLocks extends Command
{
    ...
    public function handle(): int
    {
        ...
        // Redis store supports key listing; other stores do not.
        if (!method_exists($storeImpl, 'connection')) {
            $this->error('This command requires the Redis cache store. Current store does not expose connection().');
            $this->line('Fallback: php artisan cache:clear (clears everything, not just schedule locks)');
            return self::FAILURE;
        }
