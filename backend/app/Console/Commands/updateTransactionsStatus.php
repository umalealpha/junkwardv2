...
class updateTransactionsStatus extends Command
{
  ...
  {
      $cron = new CronStatus();
      $cron->name  = "updateTransactionsStatus:cron";
      ...
      if(count($ledger) > 0)
      {
        foreach ($ledger as $lData) 
        {
          ...
          $getDay   =  date('d', strtotime($invoiceDate));
            ...
            if($premium_freq == 1)
            {
              $getLedgerData = PaymentTransactionArchive::where('policy_id',$policyId)
            ...
            if(count($getLedgerData) > 1)
            {
                ...
                foreach ($getLedgerData as  $ledgerPaymentArch) 
                {
                  ...
                  $status     = isset($ledgerPaymentArch['status']) ? $ledgerPaymentArch['status'] : '';
                  ...
                  else if($newTermDateMatch == $newLedgerNextDateMatch && $premium_freq == 3 && $isRefund == 0 && count($checkExit) == 0)
                  {
                        ...
                        if($lStatus)
                          {
                            PaymentTransactionArchive::where('id',$archID)->update(['policyStatus'=>'1']);
                          }
                  }
  ...
  {
    $totalSum = PaymentTransactionArchive::where('policy_id',$policyId)
