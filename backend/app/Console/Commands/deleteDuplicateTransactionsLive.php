...
class deleteDuplicateTransactionsLive extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'deleteDuplicateTransactionsLive:cron';
    ...
    public function handle()
    {
        ...
        try{
            $duplicateTransactions = DB::select(DB::raw("SELECT COUNT(referenceNumber),policy_id,referenceNumber FROM payment_transactions_sonali       where referenceNumber != '' GROUP BY policy_id,referenceNumber HAVING COUNT(*) > 1"));
            
            if(count($duplicateTransactions) > 0)
            {
                foreach ($duplicateTransactions as $key => $value) {
                    $getData = PaymentTransaction::where('referenceNumber', $value->referenceNumber)
                    ->where('policy_id', $value->policy_id)
                    ->get(array('id'));
                    foreach ($getData as  $gValue) {
                        $deleteEntry = PaymentTransaction::where('id',$gValue->id)->delete();
                        sleep(1);
                    }
