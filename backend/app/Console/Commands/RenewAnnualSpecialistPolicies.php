...
/**
 * Anniversary renewal for SPECIALIST products only.
 *
 * This is the specialist twin of RenewAnnualPolicies (policy:renew-annual,
 * scoped to DOM/COM motor products 7/8). It is intentionally a separate
 * command so the two never overlap and either can be scheduled / triggered
 * independently:
 *
 *   - policy:renew-annual            → products 7, 8            (DOM/COM motor)
 *   - policy:renew-annual-specialist → products 16,17,18,19,20,22 (specialist)
 *
 * Specialist products (Engineering COM/DOM, Specialist COM/DOM, Commercial
 * Liabilities, Marine — see AlphaDirect\Support\KycDomComProducts) carry their
 * coverages in dedicated one-to-one tables (car/par/ear/travel/marine/…)
 * that the plain newPolicyAction() replicator does NOT clone. We therefore
 * clone the action with newPolicyActionReplace(), which is the only path that
 * also walks SpecialistCoverageRegistry::tableNames() via
 * replicateSpecialistCoveragesIfMissing(). Premium is then re-rated by the
 * canonical recipe inside calculatePremiumRenew() so the specialist schedule
 * sums correctly. All other anniversary semantics mirror RenewAnnualPolicies.
 */
class RenewAnnualSpecialistPolicies extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'policy:renew-annual-specialist {--policy= : Restrict the run to a single policy (id or policyNumber) — for local/testing. Omit to run the full batch as the scheduler does.}';
    protected $description = 'Renew specialist (non DOM/COM motor) annual policies if eligible';

    /**
     * Specialist product ids handled by this cron. Deliberately disjoint from
     * the DOM/COM motor ids (7, 8) that policy:renew-annual handles, so a
    ...
    {
        $cron = new CronStatus();
        $cron->name = "policy:renew-annual-specialist";
        ...
        try {
            ...
            \Log::error("RenewAnnualSpecialistPolicies Cron failed: " . $e->getMessage(), [
    ...
    private function doHandle(CronStatus $cron)
    {
        ...
        $policyData = DB::table('policies as p')
            ...
            ->where('p.status', '!=', 2)
            // 90-day window only on the full scheduled batch; skipped for a
            // single-policy manual run so a missed anniversary can be caught.
            ->when(empty($singlePolicy), function ($q) {
                $q->whereRaw('CURDATE() >= DATE_SUB(DATE_ADD(last_action.effective_from, INTERVAL 1 YEAR), INTERVAL 90 DAY)');
            })
        ...
        Log::info('start cron here Specialist Anniversary. Eligible policies: ' . count($policyData));
        $finalData = array();
        foreach ($policyData as $policy) {
            ...
            // 7️⃣ Get last premium source. Seed value only —
            //    calculatePremiumRenewSpecialist below recomputes premium off the
            //    cloned schedule and overwrites it. ENDORSE stays excluded here
            //    because an endorsement's own premium is a pro-rata delta, not a
            //    full-period figure. Bounded to periods that start BEFORE the
            //    anniversary and ordered (effective_from, id): without the id
            //    tie-break two actions on the same date came back in undefined
            //    order.
            ...
            $anniversary = PolicyAction::create([
                ...
                'status' => 'QUOTE',
                'note' => 'Specialist Anniversary Renewal generated by cron (90 days prior)',
            ]);

            // 🔁 Clone + calculate premium.
            //    newPolicyActionReplaceSpecialist() is the SPECIALIST-only
            //    replicator (deliberately separate from the DOM/COM motor
            //    newPolicyActionReplace()): it makes a verbatim replica of the
            //    source action AND clones the ten specialist one-to-one coverage
            //    tables via replicateSpecialistCoveragesIfMissing(), so the
            //    anniversary quote carries the full schedule from NEWBUSINESS /
            //    last ISSUED. calculatePremiumRenewSpecialist() then prices the
            //    cloned schedule with the SPECIALIST Rate-button recipe — the
            //    plain calculatePremiumRenew sums only the standard coverage
            //    tables and ignores the ten specialist one-to-one tables, so it
            //    zeroed the anniversary premium (and its invoice).
            //    Clone from the action in force at the anniversary date
            //    ($sourceAction), NOT from the date anchor.
            PolicyAction::newPolicyActionReplaceSpecialist($anniversary, $sourceAction->id);

            PolicyAction::calculatePremiumRenewSpecialist(
                $anniversary->id,
                $baseAction->term_id,
                $policy->policy_id
            );

            Log::info("SPECIALIST ANNIVERSARY created | Policy {$policy->policy_id} | {$anniversaryFrom}");
        }
        ...
        $path = 'Policy/PoliciesAnniversaryRenewSpecialist-' . $date . '.pdf';
        $data = [
            'finalData' => $finalData
        ];
        ...
        $cron->end = \Carbon\Carbon::now();
        $cron->save();
        $this->info('Specialist annual policy renewal task completed.');
        return 0;
    }
