...
/**
 * Fix `getKYCDataApprovedRejected` — restore agent-level aggregation.
 *
 * Same regression as getAllKYCDocumentsByDate (see sibling migration
 * 2026_06_29_120001): the 2026_06_11 rewrite dropped the `count`
 * aggregate + GROUP BY, so DailyKYCReport's $action total computed to 0
 * and the "actions for today" / action-rate sections rendered empty.
 *
 * Restores one row per agent + COUNT(DISTINCT ck.id) while keeping the
 * DomCom join, the extended doc-non-null filter, and the
 * Approve-OR-Unapprove status filter.
 */
