CREATE DEFINER=`graphitebwlive`@`%` PROCEDURE `summary_age_analyst_dom_2025`(in fd VARCHAR(110))
BEGIN
   ...
   -- and c.company_id IS NULL
	...
	-- truncate summary_age_analyst_report_dom_com_2025;
...
open po;
       getPo: LOOP
			...
			AND deleted_at IS  NULL
			AND status = "Success"
			AND CompanyRef IS NOT NULL
            and CompanyRef = 'Reversed'
			AND amount != 1
			AND is_refund = 0
			AND DATE(paymentDate) <= fd
			AND DATEDIFF(fd, DATE(paymentDate)) BETWEEN 0 AND 30;
 
			SET temp_days_30 =  temp_days_30 - (@payments_30 - @refund_30);
			...
			SET temp_days_60 = temp_days_60 - (@payments_60 - @refund_60);
			...
			SET temp_days_90 = temp_days_90 - (@payments_90 - @refund_90);
			...
			SET temp_days_120 = temp_days_120 - (@payments_120 - @refund_120);
			...
			-- SET temp_balance_outstanding = ((CAST(xinvoiceTotal AS DECIMAL(16,2)) + CAST(archive_xinvoiceTotal AS DECIMAL(16,2))) - (CAST(xpaymentTotal AS DECIMAL(16,2)) + CAST(archive_xpaymentTotal AS DECIMAL(16,2)))) ;
				SET temp_balance_outstanding = (
				(CAST(xinvoiceTotal AS DECIMAL(16,2)) + CAST(archive_xinvoiceTotal AS DECIMAL(16,2)))
				- (
					(CAST(xpaymentTotal AS DECIMAL(16,2)) + CAST(archive_xpaymentTotal AS DECIMAL(16,2)))
					- (CAST(xrefundTotal AS DECIMAL(16,2)) + CAST(archive_xrefundTotal AS DECIMAL(16,2)))
				  )
				);
			SET finished = 0;
 
			IF xpolicyStatus = 'Cancelled' AND CAST(temp_xinvoiceTotal AS DECIMAL(10,2)) < CAST(temp_xpaymentTotal AS DECIMAL(10,2)) THEN
				INSERT INTO summary_age_analyst_report_dom_com_2025 (
