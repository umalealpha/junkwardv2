...
return new class extends Migration {
    ...
    {
        DB::unprepared('
     CREATE DEFINER=`graphitebwlive`@`%` PROCEDURE `temp_start_end_summary_age_analyst_dom`(IN fd VARCHAR(110),IN fd1 VARCHAR(110))
BEGIN 
   ...
   and DATE(policy_actions.effective_from) between fd and fd1
...
open po;
       getPo: LOOP
            ...
            AND DATE(pl.invoice_date) between fd and fd1
          ...
          #  AND ( date(pl.deleted_at) >= "2025-08-23" or pl.deleted_at is null)
            ...
            AND DATEDIFF(fd1, DATE(pl.invoice_date)) BETWEEN 0 AND 30
           ...
           and DATE(paymentDate) between fd and fd1
           # and created_at  <= "2025-08-23"  and ( date(deleted_at) >= "2025-08-23" or deleted_at is null) 
            ...
            AND DATEDIFF(fd1, DATE(paymentDate)) BETWEEN 0 AND 30;
           ...
           # AND DATE(pl.accounting_date) <= "2025-08-23"
            ...
            AND DATEDIFF(fd1, DATE(pl.invoice_date)) BETWEEN 31 AND 60
			AND cn.invoice_no IS NULL;
            ...
            AND DATEDIFF(fd1, DATE(paymentDate)) BETWEEN 31 AND 60;
            ...
            and DATE(invoice_date) between fd and fd1
			AND DATEDIFF(fd1, DATE(invoice_date)) BETWEEN 0 AND 30;
            ...
            AND DATEDIFF(fd1, DATE(pl.invoice_date)) BETWEEN 61 AND 90
			AND cn.invoice_no IS NULL;
            ...
            AND DATEDIFF(fd1, DATE(paymentDate)) BETWEEN 61 AND 90;
            ...
            AND DATEDIFF(fd1, DATE(invoice_date)) BETWEEN 61 AND 90;
            ...
            AND DATEDIFF(fd1, DATE(pl.invoice_date)) > 90
			AND cn.invoice_no IS NULL;
            ...
            AND DATEDIFF(fd1, DATE(invoice_date)) > 90;
            ...
            AND DATE(paymentDate) between fd and fd1
			#AND DATEDIFF(fd, DATE(paymentDate)) > 90;
            AND DATEDIFF(fd1, DATE(paymentDate)) > 90;
			...
			set xpaymentTotal=0;
			select sum(amount) as b from payment_transactions where policyNumber=xpolicyNumber  and created_at  <= "2025-08-23"  and ( date(deleted_at) >= "2025-08-23" or deleted_at is null) and status="Success" and CompanyRef IS NULL and amount != 1 and is_refund = 0 AND DATE(paymentDate) between fd and fd1  into xpaymentTotal;
			set xrefundTotal=0;
			select if(sum(amount) is null,0,sum(amount)) as b from payment_transactions where policyNumber=xpolicyNumber  and deleted_at is null and status="Success" and CompanyRef = "Reversed" AND DATE(paymentDate) between fd and fd1  into xrefundTotal;
			...
			select if(sum(invoice_amount)is null,0,sum(invoice_amount)) as b from graphite_archive.policy_ledger where policy_id=xid and trans_type="Invoice" and status != "Reversed" AND DATE(invoice_date) between fd and fd1  into archive_xinvoiceTotal;
			set archive_xpaymentTotal=0;
			select sum(amount) as b from graphite_archive.payment_transactions where policyNumber=xpolicyNumber  and deleted_at is null and status="Success" and amount != 1 and is_refund = 0 AND DATE(paymentDate) between fd and fd1  into archive_xpaymentTotal;
			set archive_xrefundTotal=0;
			select if(sum(amount) is null,0,sum(amount)) as b from graphite_archive.payment_transactions where policyNumber=xpolicyNumber  and deleted_at is null and status="Success" and is_refund = 1 AND DATE(paymentDate) between fd and fd1  into archive_xrefundTotal;
			...
			IF xpolicyStatus = "Cancelled"  THEN
				INSERT INTO temp_start_end_summary_age_analyst_com (
...
END
   ...
   public function down()
    {
        DB::unprepared("DROP PROCEDURE IF EXISTS temp_start_end_summary_age_analyst_dom");
    }
};