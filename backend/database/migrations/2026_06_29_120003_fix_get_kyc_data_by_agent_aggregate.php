...
/**
 * Fix `getKYCDataByAgent` — restore agent-level aggregation.
 *
 * Same regression as the sibling SPs (see 2026_06_29_120001): the
 * 2026_06_11 rewrite returned detail rows with no `count` column, so
 * the $unchecked / $approved / $unapproved totals in DailyKYCReport
 * all computed to 0 and every Blade section rendered "No documents".
 *
 * Restores one row per agent + COUNT(DISTINCT ck.id) while keeping the
 * DomCom join, the extended doc-non-null filter, and the IN-parameter
 * status filter (`ck.status = inStatus`).
 *
 * NOTE on `reason`: the 'Unapprove' call feeds the rejected section,
 * which prints `{!! $un->reason !!}` per agent row. Because we now
 * aggregate per agent, an agent with several rejected docs may have
 * multiple reasons — these are collapsed with GROUP_CONCAT (DISTINCT,
 * <br>-separated; Blade renders it unescaped so the breaks show).
 */
