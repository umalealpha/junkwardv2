CREATE TABLE IF NOT EXISTS `summary_age_analyst_report_erection_all_risk` (
...
DROP PROCEDURE IF EXISTS `summary_age_analyst_erection_all_risk`;

CREATE DEFINER=`graphitebwlive`@`%` PROCEDURE `summary_age_analyst_erection_all_risk`(in fd VARCHAR(110))
BEGIN
   ...
   END AS client_name,
    ...
    and pr.id IN (16)
    AND tb_cvgpccoverages.s_CoverageCode = 'ERECTIONALLRISKS'
	AND p.status != '2'
    group by p.id;
	...
	OPEN po;
       getPo: LOOP
			...
			IF xpolicyStatus = 'Cancelled' AND CAST(temp_xinvoiceTotal AS DECIMAL(10,2)) < CAST(temp_xpaymentTotal AS DECIMAL(10,2)) THEN
				INSERT INTO summary_age_analyst_report_erection_all_risk (
