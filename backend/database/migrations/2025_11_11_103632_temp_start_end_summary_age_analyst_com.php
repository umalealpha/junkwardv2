...
use Illuminate\Support\Facades\DB;
 ...
 return new class extends Migration {
    ...
    {
        DB::unprepared('
        CREATE DEFINER=`graphitebwlive`@`%` PROCEDURE `temp_start_end_summary_age_analyst_com`(IN fd VARCHAR(110),IN fd1 VARCHAR(110))
BEGIN
   ...
   and DATE(policy_actions.effective_from) between fd and fd1
	...
	OPEN po;
       getPo: LOOP
			...
			AND DATE(invoice_date) between fd and fd1
			AND DATEDIFF(fd1, DATE(invoice_date)) BETWEEN 31 AND 60;
		...
		#	select sum(amount) as b from payment_transactions where policyNumber=xpolicyNumber and deleted_at is null and status="Success" and CompanyRef IS NULL and amount != 1 and is_refund = 0 and date(paymentDate) <= fd into xpaymentTotal;
            select sum(amount) as b from payment_transactions where policyNumber=xpolicyNumber and deleted_at is null and status="Success" and CompanyRef IS NULL and amount != 1 and is_refund = 0 AND DATE(paymentDate) between fd and fd1 into xpaymentTotal;
			set xrefundTotal=0;  
...
END');
  ...
  public function down()
    {
        DB::unprepared("DROP PROCEDURE IF EXISTS temp_start_end_summary_age_analyst_com");
    }
};
