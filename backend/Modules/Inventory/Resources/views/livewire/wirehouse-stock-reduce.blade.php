<div x-data="{ warehouses_id: @entangle('warehouses_id'),product_id: @entangle('product_id'),plan_id: @entangle('plan_id'),reason: @entangle('reason') }"
     ...
     <div class="row">
		...
		<div class="col-sm-6">
			<div class="form-group">
				...
				<select class="form-control @error('product_id') is-invalid @enderror" wire:model="product_id">
					<option value="">- Select -</option>
					@if($warehouses_id!="")
					@foreach($this->getproduct() as $k=>$v)
		...
		<div class="col-sm-6">
			<div class="form-group">
				...
				<select x-on:change="$wire.stock()" class="form-control @error('plan_id') is-invalid @enderror" wire:model="plan_id">
					<option value="">- Select Product Plan -</option>
					@foreach($this->getProductPlan() as $p)
						<option value="{{$p->plan->id ?? ''}}"> {{$p->plan->name ?? ''}}</option>
					@endforeach
				</select>
				<div class="error invalid-feedback">
		...
		<div class="col-sm-6">
			<div class="form-group">
				...
				<select class="form-control @error('reason') is-invalid @enderror" wire:model="reason">
					...
					<option value="transfer"> Transfer to Other Warehouse</option>
				</select>
				<div class="error invalid-feedback">
			...
			<div class="row">
				...
				<div class="col-sm-6">
					<div class="form-group" x-show="reason=='transfer'">
						...
						<select class="form-control @error('transfer_warehouses_id') is-invalid @enderror" wire:model.defer="transfer_warehouses_id">
							<option value="">- Select -</option>
				...
				<div class="kt-form__actions">
					<div class="row">
						<div class="col-12 text-center">
							...
							<a class="btn btn-danger" wire:offline.attr="disabled" wire:loading.attr="disabled" href="{{route('inventory.warehouse.stock-reduce')}}">
								Cancel
							</a>
						</div>
